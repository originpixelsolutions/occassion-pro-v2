...
describe('RLS on crew_pool (Phase 12 Unit 80)', () => {
  ...
  it('member sees own tenant crew only', async () => {
    ...
    await db.query(`INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1,'Alice', FALSE, TRUE, 0)`, [t1]);
    await db.query(`INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1,'Bob', FALSE, TRUE, 0)`, [t2]);
    ...
    const names = (await db.query<{ full_name: string }>(`SELECT full_name FROM crew_pool`)).rows.map(r => r.full_name);
  ...
  it('anon sees zero', async () => {
    ...
    await db.query(`INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1,'Z', FALSE, TRUE, 0)`, [t]);
  ...
  it('owner can INSERT', async () => {
    ...
    await asRole(db, 'authenticated');
    await db.query(`INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1,'New', FALSE, TRUE, 0)`, [t]);
    await asSuperuser(db);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM crew_pool`)).rows[0]!.c).toBe(1);
  });
  ...
  it('team_member cannot INSERT', async () => {
    ...
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1,'X', FALSE, TRUE, 0)`, [t]); }
