...
describe('module_permissions — schema correctness (Phase 2 Unit 23)', () => {
  ...
  it('inserts a valid row', async () => {
    ...
    await db.query(
      `INSERT INTO module_permissions (tenant_id, role, module, can_read)
       VALUES ($1, 'event_manager', 'events', TRUE)`, [t]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM module_permissions`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('composite PK blocks duplicate (tenant, role, module)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO module_permissions (tenant_id, role, module, can_read)
       VALUES ($1, 'event_manager', 'events', FALSE)`, [t]);
  ...
  it('rejects bogus role (owner is implicit, not stored)', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO module_permissions (tenant_id, role, module) VALUES ($1, 'owner', 'events')`, [t]);
  ...
  it('rejects bogus module', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO module_permissions (tenant_id, role, module) VALUES ($1, 'team_lead', 'unicorns')`, [t]);
  ...
  it('accepts full bundle with can_read=true', async () => {
    ...
    await db.query(
      `INSERT INTO module_permissions (tenant_id, role, module, can_read, can_write, can_delete, can_export)
       VALUES ($1, 'event_manager', 'events', TRUE, TRUE, TRUE, TRUE)`, [t]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM module_permissions`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('CASCADE: deleting tenant removes its permissions', async () => {
    ...
    await db.query(
      `INSERT INTO module_permissions (tenant_id, role, module, can_read)
       VALUES ($1, 'team_member', 'events', TRUE)`, [t]);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM module_permissions`)).rows.length);
    expect(anon).toBe(0);
