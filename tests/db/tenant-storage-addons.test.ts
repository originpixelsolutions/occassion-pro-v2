...
describe('tenant_storage_addons — schema correctness (Phase 2 Unit 10)', () => {
  ...
  it('inserts an active subscription', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id) VALUES ($1, $2)`, [t, a]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_storage_addons`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects quantity = 0 and quantity > 100', async () => {
    ...
    const e1 = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, quantity) VALUES ($1, $2, 0)`, [t, a]);
    expect(e1).toMatch(/qty_bounds|quantity|check/i);
    const e2 = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, quantity) VALUES ($1, $2, 101)`, [t, a]);
  ...
  it('rejects bogus status', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, status) VALUES ($1, $2, 'frozen')`, [t, a]);
  ...
  it("rejects 'cancelled' without cancelled_at", async () => {
    ...
    const a = await mkAddon(db, 'small', 50);
    const err = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, status) VALUES ($1, $2, 'cancelled')`, [t, a]);
  ...
  it('rejects cooldown earlier than cancelled_at', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, status, cancelled_at, cancellation_cooldown_until)
       VALUES ($1, $2, 'cancelled', now(), now() - interval '1 day')`, [t, a]);
  ...
  it('rejects period_end <= period_start', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, current_period_start, current_period_end)
       VALUES ($1, $2, now(), now() - interval '1 hour')`, [t, a]);
  ...
  it('partial UNIQUE: a cancelled sub does not block a new active one', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id, status, cancelled_at)
       VALUES ($1, $2, 'cancelled', now())`, [t, a]);
    await db.query(
      `INSERT INTO tenant_storage_addons (tenant_id, addon_id) VALUES ($1, $2)`, [t, a]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_storage_addons WHERE tenant_id = $1`, [t])).rows[0]!.c;
    expect(c).toBe(2);
  });
