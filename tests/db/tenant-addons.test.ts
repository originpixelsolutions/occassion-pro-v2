...
describe('tenant_addons — schema correctness (Phase 2 Unit 11)', () => {
  ...
  it('inserts an active subscription', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_addons (tenant_id, addon_id) VALUES ($1, $2)`, [t, a]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_addons`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects quantity = 0 and quantity > 1000', async () => {
    ...
    const e1 = await tryExec(db,
      `INSERT INTO tenant_addons (tenant_id, addon_id, quantity) VALUES ($1, $2, 0)`, [t, a]);
    expect(e1).toMatch(/qty_bounds|quantity|check/i);
    const e2 = await tryExec(db,
      `INSERT INTO tenant_addons (tenant_id, addon_id, quantity) VALUES ($1, $2, 1001)`, [t, a]);
  ...
  it('rejects bogus status', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_addons (tenant_id, addon_id, status) VALUES ($1, $2, 'frozen')`, [t, a]);
  ...
  it("rejects 'cancelled' without cancelled_at", async () => {
    ...
    const a = await mkAddon(db, 'extra_events');
    const err = await tryExec(db,
      `INSERT INTO tenant_addons (tenant_id, addon_id, status) VALUES ($1, $2, 'cancelled')`, [t, a]);
  ...
  it('cancelled sub does not block a new active one', async () => {
    ...
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_addons WHERE tenant_id = $1`, [t])).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM tenant_addons`)).rows.length);
    expect(anon).toBe(0);
