...
describe('event_edit_sessions — schema correctness (Phase 3 Unit 6)', () => {
  ...
  it('inserts a valid lock with default 60s expiry', async () => {
    ...
    await db.query(
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, 'name')`, [e, m]);
  ...
  it('rejects empty field_path', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, '   ')`, [e, m]);
  ...
  it('rejects expiry > 1 hour from locked_at (heartbeat ceiling)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path, expires_at)
       VALUES ($1, $2, 'name', now() + interval '90 minutes')`, [e, m]);
  ...
  it('rejects bogus released_reason', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path, released_at, released_reason)
       VALUES ($1, $2, 'name', now(), 'unicorn_escape')`, [e, m]);
  ...
  it('partial UNIQUE: blocks two ACTIVE locks on same (event, field)', async () => {
    ...
    await db.query(
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, 'name')`, [e, m1]);
    const err = await tryExec(db,
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, 'name')`, [e, m2]);
  ...
  it('released lock does NOT block a new lock on same (event, field)', async () => {
    ...
    await db.query(
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path, released_at, released_reason)
       VALUES ($1, $2, 'name', now(), 'user')`, [e, m1]);
    await db.query(
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, 'name')`, [e, m2]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_edit_sessions`)).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('trigger: rejects cross-tenant lock attempt', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_edit_sessions (event_id, user_id, field_path) VALUES ($1, $2, 'name')`, [e_t1, m_t2]);
