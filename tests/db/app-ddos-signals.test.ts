...
describe('app_ddos_signals — schema correctness (Phase 2 Unit 41)', () => {
  ...
  it('inserts a valid signal', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM app_ddos_signals`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('SET NULL: deleting tenant nulls tenant_id but keeps row (audit trail)', async () => {
    ...
    await db.query(
      `INSERT INTO app_ddos_signals (signal_type, count, tenant_id) VALUES ('api_abuse', 200, $1)`, [t]);
    await db.query(`DELETE FROM tenants WHERE id = $1`, [t]);
    const r = await db.query<{ tenant_id: string | null }>(`SELECT tenant_id FROM app_ddos_signals`);
  ...
  it('RLS pair', async () => {
    await db.query(
      `INSERT INTO app_ddos_signals (signal_type, count) VALUES ('rate_burst', 1)`);
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM app_ddos_signals`)).rows.length);
    expect(anon).toBe(0);
