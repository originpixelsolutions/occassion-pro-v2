...
describe('vendor_reviews — schema correctness (Phase 4 Unit 44)', () => {
  ...
  it('inserts a valid review', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_reviews (vendor_account_id, reviewer_tenant_id, event_id, vendor_assignment_id, rating, review_text, professionalism_score, quality_score, value_score)
       VALUES ($1,$2,$3,$4, 4.5, 'Great work', 4.5, 5.0, 4.0)`, [v, t, e, a]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_reviews`)).rows[0]!.c).toBe(1);
  });

  it('rejects rating < 1.0', async () => {
    const v = await mkVendor(db, 'v2@y.dev');
    const err = await tryExec(db,
      `INSERT INTO vendor_reviews (vendor_account_id, rating) VALUES ($1, 0.5)`, [v]);
  ...
  it('rejects rating > 5.0', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_reviews (vendor_account_id, rating) VALUES ($1, 6.0)`, [v]);
    expect(err).toMatch(/rating|check/i);
  });
  ...
  it('partial UNIQUE: one review per assignment', async () => {
    ...
    const a = await mkAssignment(db, t, e, v);
    await db.query(`INSERT INTO vendor_reviews (vendor_account_id, reviewer_tenant_id, event_id, vendor_assignment_id, rating) VALUES ($1,$2,$3,$4, 4.0)`, [v, t, e, a]);
    const err = await tryExec(db,
      `INSERT INTO vendor_reviews (vendor_account_id, reviewer_tenant_id, event_id, vendor_assignment_id, rating) VALUES ($1,$2,$3,$4, 5.0)`, [v, t, e, a]);
  ...
  it('NULL assignment_id: multiple reviews allowed', async () => {
    ...
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_reviews`)).rows[0]!.c).toBe(2);
  });
  ...
  it('vendor_response two-way coupling: text without timestamp rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_reviews (vendor_account_id, rating, vendor_response)
       VALUES ($1, 4.0, 'Thanks for the feedback')`, [v]);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM vendor_reviews`)).rows.length);
    expect(anon).toBe(0);
