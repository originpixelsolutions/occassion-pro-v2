...
async function mkSysEventType(db: TestDb, code: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES (NULL, $1, 'Sys', TRUE) RETURNING id`, [code])).rows[0]!.id;
}
async function mkTenantEventType(db: TestDb, tenantId: string, code: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES ($1, $2, 'Cust', FALSE) RETURNING id`, [tenantId, code])).rows[0]!.id;
}
...
describe('RLS on event_type_readiness_items (Phase 12 Unit 84)', () => {
  ...
  it('authenticated member sees system readiness items', async () => {
    ...
    await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'Venue confirmed', 1, 1)`, [sysEt]);
    ...
    const n = (await db.query<{ id: string }>(`SELECT id FROM event_type_readiness_items`)).rows.length;
  ...
  it('anon CANNOT see system readiness items', async () => {
    const sysEt = await mkSysEventType(db, 'wedding');
    await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'X', 1, 1)`, [sysEt]);
  ...
  it('tenant items are scoped to own tenant', async () => {
    ...
    await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'A', 1, 1)`, [et1]);
    await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'B', 1, 1)`, [et2]);
    const u = '00000000-0000-0000-0000-000000001710';
    await mkMember(db, t1, u, 'm@y.dev');
    await setCtx(db, u, 'tenant_member', t1);
    await asRole(db, 'authenticated');
    const labels = (await db.query<{ label: string }>(`SELECT label FROM event_type_readiness_items`)).rows.map(r => r.label);
    await asSuperuser(db);
  ...
  it('owner can INSERT tenant readiness item', async () => {
    ...
    await setCtx(db, u, 'tenant_member', t);
    await asRole(db, 'authenticated');
    await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'New', 1, 1)`, [et]);
    await asSuperuser(db);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_type_readiness_items WHERE label='New'`)).rows[0]!.c).toBe(1);
  });
  ...
  it('tenant member cannot INSERT against a system event_type', async () => {
    ...
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO event_type_readiness_items (event_type_id, label, weight, sort_order) VALUES ($1,'Sneak', 1, 1)`, [sysEt]); }
