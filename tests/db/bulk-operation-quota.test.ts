...
describe('bulk_operation_quota — schema correctness (Phase 2 Unit 45)', () => {
  ...
  it('inserts a valid quota row', async () => {
    ...
    await db.query(
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date, count, limit_value)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date, 0, 10000)`,
      [t, EVENT],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM bulk_operation_quota`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects bogus operation_type', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date)
       VALUES ($1, 'hex_curse', 'per_event', $2, current_date)`,
      [t, EVENT],
    );
  ...
  it('rejects bogus scope', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date)
       VALUES ($1, 'guest_import', 'per_galaxy', $2, current_date)`,
      [t, EVENT],
    );
  ...
  it('rejects negative count', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date, count)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date, -1)`,
      [t, EVENT],
    );
  ...
  it('rejects count > limit_value', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date, count, limit_value)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date, 10001, 10000)`,
      [t, EVENT],
    );
  ...
  it('atomic increment respects the cap', async () => {
    ...
    await db.query(
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date, count, limit_value)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date, 9999, 10000)`,
      [t, EVENT],
    );
    // +1 OK
    await db.query(`UPDATE bulk_operation_quota SET count = count + 1 WHERE tenant_id = $1`, [t]);
    // +1 again trips the CHECK
    const err = await tryExec(
      db,
      `UPDATE bulk_operation_quota SET count = count + 1 WHERE tenant_id = $1`,
      [t],
    );
    expect(err).toMatch(/count_under_limit|check/i);
  });
  ...
  it('composite PK: same (tenant, op, scope, scope_id, date) blocked', async () => {
    ...
    await db.query(
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date)`,
      [t, EVENT],
    );
    const err = await tryExec(
      db,
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date, count)
       VALUES ($1, 'guest_import', 'per_event', $2, current_date, 5)`,
      [t, EVENT],
    );
  ...
  it('different scope_ids, dates, op types all coexist', async () => {
    ...
    await db.query(
      `INSERT INTO bulk_operation_quota (tenant_id, operation_type, scope, scope_id, date) VALUES
         ...
         ($1, 'email_send',   'per_workspace', $1, current_date)`,
      [t, EVENT, EVENT2],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM bulk_operation_quota`))
      .rows[0]!.c;
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      ...
      'anon',
      async () =>
        (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM bulk_operation_quota`)).rows
          .length,
    );
    expect(anon).toBe(0);
