...
describe('tenant_member_external_calendars — schema correctness (Phase 2 Unit 22)', () => {
  ...
  it('inserts a valid calendar', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted)
       VALUES ($1, 'google_calendar', '\\x00aa'::bytea)`, [m]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_member_external_calendars`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects bogus provider', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted)
       VALUES ($1, 'fastmail', '\\x00aa'::bytea)`, [m]);
  ...
  it('rejects bogus sync_direction', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted, sync_direction)
       VALUES ($1, 'google_calendar', '\\x00aa'::bytea, 'one_way')`, [m]);
  ...
  it('rejects empty access_token', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted)
       VALUES ($1, 'google_calendar', ''::bytea)`, [m]);
  ...
  it("rejects 'expired' without token_expires_at", async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted, status)
       VALUES ($1, 'google_calendar', '\\x00aa'::bytea, 'expired')`, [m]);
  ...
  it('partial UNIQUE: one active calendar per (member, provider)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted)
       VALUES ($1, 'google_calendar', '\\x00bb'::bytea)`, [m]);
  ...
  it('disconnected calendar does not block a new active one', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted, status)
       VALUES ($1, 'google_calendar', '\\x00aa'::bytea, 'disconnected')`, [m]);
    await db.query(
      `INSERT INTO tenant_member_external_calendars (member_id, provider, access_token_encrypted)
       VALUES ($1, 'google_calendar', '\\x00bb'::bytea)`, [m]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_member_external_calendars WHERE member_id = $1`, [m])).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM tenant_member_external_calendars`)).rows.length);
    expect(anon).toBe(0);
