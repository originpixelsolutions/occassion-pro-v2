...
describe('vendor_invoice_templates — schema correctness (Phase 4 Unit 42)', () => {
  ...
  it('inserts a valid HTML template', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html, default_currency_code, is_default)
       VALUES ($1,'Standard Invoice','html','<html><body><h1>Invoice</h1></body></html>','INR', TRUE)`,
      [v],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_invoice_templates`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('inserts a valid PDF overlay', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_file_r2_key, template_file_size_bytes)
       VALUES ($1,'Letterhead PDF','pdf_overlay','vendors/abc/template.pdf', 524288)`,
      [v],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_invoice_templates`))
  ...
  it('html type without template_html rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type) VALUES ($1,'X','html')`,
      [v],
    );
  ...
  it('pdf_overlay without r2_key rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type) VALUES ($1,'X','pdf_overlay')`,
      [v],
    );
  ...
  it('docx without r2_key rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type) VALUES ($1,'X','docx')`,
      [v],
    );
    expect(err).toMatch(/file_requires_key|check/i);
  });
  ...
  it('rejects bad template_type', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html) VALUES ($1,'X','markdown','# X')`,
      [v],
    );
  ...
  it('rejects bad sha256 hash', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_file_r2_key, template_file_hash_sha256)
       VALUES ($1,'X','pdf_overlay','vendors/x.pdf','NOT-A-HASH')`,
      [v],
    );
  ...
  it('rejects non-https default_logo_url', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html, default_logo_url)
       VALUES ($1,'X','html','<html></html>','http://insecure/logo.png')`,
      [v],
    );
  ...
  it('partial UNIQUE: only one default per vendor', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html, is_default) VALUES ($1,'A','html','<html></html>', TRUE)`,
      [v],
    );
    const err = await tryExec(
      db,
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html, is_default) VALUES ($1,'B','html','<html></html>', TRUE)`,
      [v],
    );
  ...
  it('partial UNIQUE: case-fold name within vendor', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html) VALUES ($1,'Standard Invoice','html','<html></html>')`,
      [v],
    );
    const err = await tryExec(
      db,
  ...
  it('retired with is_active=TRUE rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html, is_active, retired_at, retired_reason)
       VALUES ($1,'X','html','<html></html>', TRUE, now(), 'replaced')`,
      [v],
    );
  ...
  it('RLS pair', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_invoice_templates (vendor_account_id, name, template_type, template_html) VALUES ($1,'X','html','<html></html>')`,
      [v],
    );
