...
describe('vendor_portfolios — schema correctness (Phase 4 Unit 43)', () => {
  ...
  it('inserts a valid portfolio', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_portfolios (vendor_account_id, about_text, service_categories, service_regions, starting_price, starting_currency, years_in_business, slug, visibility)
       VALUES ($1,'Premium caterer', ARRAY['catering','live_cooking'], ARRAY['Mumbai','Pune','Goa'], 250000.00, 'INR', 12, 'acme-catering', 'public')`,
      [v],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_portfolios`)).rows[0]!
        .c,
    ).toBe(1);
  });
  ...
  it('PK = vendor_account_id: one portfolio per vendor', async () => {
    ...
    const err = await tryExec(db, `INSERT INTO vendor_portfolios (vendor_account_id) VALUES ($1)`, [
      v,
    ]);
  ...
  it('starting_price without currency rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, starting_price) VALUES ($1, 1000)`,
      [v],
    );
  ...
  it('rating without count rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, avg_performance_rating, total_ratings_count) VALUES ($1, 4.5, 0)`,
      [v],
    );
  ...
  it('rating + count coupling: count > 0 with NULL rating rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, total_ratings_count) VALUES ($1, 10)`,
      [v],
    );
    expect(err).toMatch(/rating_count|check/i);
  });
  ...
  it('rejects bad visibility', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, visibility) VALUES ($1, 'admin_only')`,
      [v],
    );
  ...
  it('UNIQUE slug across vendors', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_portfolios (vendor_account_id, slug) VALUES ($1,'acme-catering')`,
      [v1],
    );
    const err = await tryExec(
      db,
  ...
  it('rejects gallery > 20 images', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, gallery_image_urls) VALUES ($1, $2::text[])`,
      [v, urls],
    );
  ...
  it('verified requires verified_at + verified_by', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_portfolios (vendor_account_id, is_verified) VALUES ($1, TRUE)`,
      [v],
    );
  ...
  it('verified happy path with super admin', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_portfolios (vendor_account_id, is_verified, verified_at, verified_by) VALUES ($1, TRUE, now(), $2)`,
      [v, sa],
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM vendor_portfolios WHERE is_verified=TRUE`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      ...
      async () =>
        (
          await db.query<{ vendor_account_id: string }>(
            `SELECT vendor_account_id FROM vendor_portfolios`,
          )
        ).rows.length,
    );
    expect(anon).toBe(0);
