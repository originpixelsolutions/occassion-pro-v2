...
describe('vendor_accounts — schema correctness (Phase 3 Unit 14)', () => {
  ...
  it('inserts a valid vendor', async () => {
    await db.query(
      `INSERT INTO vendor_accounts (email, company_name, contact_name, phone, password_hash, tax_id, default_currency)
       VALUES ('v@y.dev','Acme Catering Ltd','Bob','+919876543210',$1,'TAX-001','INR')`,
      [PW],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_accounts`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });

  it('UNIQUE email blocks duplicate (citext case-fold)', async () => {
    await db.query(`INSERT INTO vendor_accounts (email, password_hash) VALUES ('v@y.dev', $1)`, [
      PW,
    ]);
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash) VALUES ('V@Y.DEV', $1)`,
      [PW],
    );
  ...
  it('rejects malformed currency', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash, default_currency) VALUES ('v@y.dev', $1, 'usd')`,
      [PW],
    );
  ...
  it('rejects bad phone', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, phone, password_hash) VALUES ('v@y.dev', '9876543210', $1)`,
      [PW],
    );
  ...
  it('mfa_coupling: enabled=TRUE without secret rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash, mfa_enabled) VALUES ('v@y.dev', $1, TRUE)`,
      [PW],
    );
  ...
  it('suspend_coupling: suspended_at without reason rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash, suspended_at) VALUES ('v@y.dev', $1, now())`,
      [PW],
    );
  ...
  it('bank_coupling: encrypted without kms_key_id rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash, bank_account_encrypted) VALUES ('v@y.dev', $1, '\\x00aabb'::bytea)`,
      [PW],
    );
  ...
  it('bank_coupling: kms_key_id without encrypted rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO vendor_accounts (email, password_hash, bank_kms_key_id) VALUES ('v@y.dev', $1, 'arn:kms:key/abc')`,
      [PW],
    );
    expect(err).toMatch(/bank|check/i);
  });
  ...
  it('bank_coupling: both present accepted', async () => {
    await db.query(
      `INSERT INTO vendor_accounts (email, password_hash, bank_account_encrypted, bank_kms_key_id)
       VALUES ('v@y.dev', $1, '\\x00aabb'::bytea, 'arn:kms:key/abc')`,
      [PW],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_accounts`))
      .rows[0]!.c;
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM vendor_accounts`)).rows.length,
    );
    expect(anon).toBe(0);
