...
describe('RLS on floor_plans chain (Phase 12 Unit 100)', () => {
  ...
  it('member sees own tenant floor_plans only', async () => {
    ...
    await db.query(
      `INSERT INTO floor_plans (tenant_id, event_id, name, canvas, is_published, version) VALUES ($1, $2, 'FP A', '{}'::jsonb, FALSE, 1)`,
      [t1, e1],
    );
    await db.query(
      `INSERT INTO floor_plans (tenant_id, event_id, name, canvas, is_published, version) VALUES ($1, $2, 'FP B', '{}'::jsonb, FALSE, 1)`,
      [t2, e2],
    );
    ...
    const names = (await db.query<{ name: string }>(`SELECT name FROM floor_plans`)).rows.map(
      (r) => r.name,
    );
  ...
  it('anon sees zero', async () => {
    ...
    await db.query(
      `INSERT INTO floor_plans (tenant_id, event_id, name, canvas, is_published, version) VALUES ($1, $2, 'X', '{}'::jsonb, FALSE, 1)`,
      [t, e],
    );
