...
describe('crew_pool — schema correctness (Phase 3 Unit 9)', () => {
  ...
  it('inserts a valid crew entry', async () => {
    ...
    await db.query(
      `INSERT INTO crew_pool (tenant_id, full_name, phone, email, role, hourly_rate, currency_code, rating)
       VALUES ($1, 'Ankur Mehta', '+919876543210', 'ankur@y.dev', 'sound', 600.00, 'INR', 4.7)`,
      [t],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM crew_pool`)).rows[0]!
      .c;
    expect(c).toBe(1);
  });
  ...
  it('rejects bad phone format (no plus)', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, phone) VALUES ($1, 'X', '9876543210')`,
      [t],
    );
  ...
  it('rejects malformed email', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, email) VALUES ($1, 'X', 'not-an-email')`,
      [t],
    );
  ...
  it('rejects bogus role', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, role) VALUES ($1, 'X', 'magician')`,
      [t],
    );
  ...
  it('rejects rate without currency', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, hourly_rate) VALUES ($1, 'X', 100)`,
      [t],
    );
  ...
  it('rejects rating outside 1.0..5.0', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, rating) VALUES ($1, 'X', 5.5)`,
      [t],
    );
  ...
  it('partial UNIQUE: same active phone per tenant blocked', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO crew_pool (tenant_id, full_name, phone) VALUES ($1, 'A', '+919876543210')`,
      [t],
    );
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, phone) VALUES ($1, 'B', '+919876543210')`,
      [t],
    );
  ...
  it('deactivating frees the phone slot', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO crew_pool (tenant_id, full_name, phone, is_active) VALUES ($1, 'A', '+919876543210', FALSE)`,
      [t],
    );
    await db.query(
      `INSERT INTO crew_pool (tenant_id, full_name, phone) VALUES ($1, 'B', '+919876543210')`,
  ...
  it('citext email: A@Y.DEV blocked by lower-case duplicate', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO crew_pool (tenant_id, full_name, email) VALUES ($1, 'A', 'a@y.dev')`,
      [t],
    );
    const err = await tryExec(
      db,
      `INSERT INTO crew_pool (tenant_id, full_name, email) VALUES ($1, 'B', 'A@Y.DEV')`,
      [t],
    );
    expect(err).toMatch(/duplicate|unique/i);
  });
  ...
  it('CASCADE: deleting tenant removes its crew', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(`INSERT INTO crew_pool (tenant_id, full_name) VALUES ($1, 'A')`, [t]);
