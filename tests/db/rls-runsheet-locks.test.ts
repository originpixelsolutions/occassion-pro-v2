...
describe('RLS on runsheet_locks (Phase 12 Unit 77)', () => {
  ...
  it('holder can INSERT their own lock', async () => {
    ...
    await db.query(`INSERT INTO runsheet_locks (event_id, tenant_id, locked_by) VALUES ($1, $2, $3)`, [e, t, u]);
    await asSuperuser(db);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM runsheet_locks`)).rows[0]!.c).toBe(1);
  });
  ...
  it('member cannot INSERT a lock in someone else name', async () => {
    ...
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO runsheet_locks (event_id, tenant_id, locked_by) VALUES ($1, $2, $3)`, [e, t, other]); }
  ...
  it('non-holder cannot UPDATE the lock', async () => {
    ...
    await db.query(`INSERT INTO runsheet_locks (event_id, tenant_id, locked_by) VALUES ($1, $2, $3)`, [e, t, holder]);
    await setCtx(db, intruder, 'tenant_member', t);
    await asRole(db, 'authenticated');
    const r = await db.query<{ c: number }>(
      `WITH u AS (UPDATE runsheet_locks SET expires_at=now() WHERE event_id=$1 RETURNING event_id) SELECT count(*)::int AS c FROM u`, [e]);
    await asSuperuser(db);
  ...
  it('manager can force-release someone else lock', async () => {
    ...
    await asRole(db, 'authenticated');
    const r = await db.query<{ c: number }>(
      `WITH d AS (DELETE FROM runsheet_locks WHERE event_id=$1 RETURNING event_id) SELECT count(*)::int AS c FROM d`, [e]);
    await asSuperuser(db);
