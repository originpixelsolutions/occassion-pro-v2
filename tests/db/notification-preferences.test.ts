...
describe('notification_preferences — schema correctness (Phase 8 Unit 54)', () => {
  ...
  it('inserts a valid preference', async () => {
    ...
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id, sms_enabled, digest_frequency, quiet_hours_start, quiet_hours_end, quiet_hours_timezone)
       VALUES (gen_random_uuid(),'client','event.updated', $1, TRUE, 'daily', '22:00','07:00','Asia/Kolkata')`,
      [t],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM notification_preferences`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('composite PK blocks dupe (user, type, category)', async () => {
    ...
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id) VALUES ($1,'client','x',$2)`,
      [uid, t],
    );
  ...
  it('different categories for same user allowed', async () => {
    ...
    const uid = (await db.query<{ id: string }>(`SELECT gen_random_uuid() AS id`)).rows[0]!.id;
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id) VALUES ($1,'client','event.updated',$2)`,
      [uid, t],
    );
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id) VALUES ($1,'client','payment.received',$2)`,
      [uid, t],
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM notification_preferences WHERE user_id=$1`,
          [uid],
        )
      ).rows[0]!.c,
    ).toBe(2);
  ...
  it('rejects bad user_type', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id) VALUES (gen_random_uuid(),'admin','x',$1)`,
      [t],
    );
  ...
  it('rejects bad digest_frequency', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id, digest_frequency) VALUES (gen_random_uuid(),'client','x',$1,'biweekly')`,
      [t],
    );
  ...
  it('quiet_hours_start without end rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id, quiet_hours_start) VALUES (gen_random_uuid(),'client','x',$1,'22:00')`,
      [t],
    );
  ...
  it('quiet_hours_end without start rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id, quiet_hours_end) VALUES (gen_random_uuid(),'client','x',$1,'07:00')`,
      [t],
    );
    expect(err).toMatch(/quiet_hours_coupling|check/i);
  });
  ...
  it('super_admin without tenant_id accepted', async () => {
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category) VALUES (gen_random_uuid(),'super_admin','platform.alerts')`,
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM notification_preferences WHERE user_type='super_admin'`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('default channel settings: in_app/email/push on, sms/whatsapp/slack/teams off', async () => {
    ...
    await db.query(
      `INSERT INTO notification_preferences (user_id, user_type, category, tenant_id) VALUES (gen_random_uuid(),'client','x',$1)`,
      [t],
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      ...
      'anon',
      async () =>
        (await db.query<{ user_id: string }>(`SELECT user_id FROM notification_preferences`)).rows
          .length,
    );
    expect(anon).toBe(0);
