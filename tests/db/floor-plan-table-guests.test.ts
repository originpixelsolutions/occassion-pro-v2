...
async function mkTable(
  ...
  return (
    await db.query<{ id: string }>(
     ...
     VALUES ($1,$2,$3,$4, 10, 0, 0) RETURNING id`,
      [tenant, plan, event, num],
    )
  ).rows[0]!.id;
}
...
describe('floor_plan_table_guests — schema correctness (Phase 3 Unit 34)', () => {
  ...
  it('inserts a valid seating assignment', async () => {
    ...
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM floor_plan_table_guests`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('composite PK blocks dupe (table, guest)', async () => {
    ...
    const g = await mkGuest(db, t, e);
    await db.query(
      `INSERT INTO floor_plan_table_guests (table_id, guest_id, tenant_id, event_id) VALUES ($1,$2,$3,$4)`,
      [tb, g, t, e],
    );
  ...
  it('partial UNIQUE blocks two guests on same seat_number of same table', async () => {
    ...
    await db.query(
      `INSERT INTO floor_plan_table_guests (table_id, guest_id, tenant_id, event_id, seat_number) VALUES ($1,$2,$3,$4, 1)`,
      [tb, g1, t, e],
    );
    const err = await tryExec(
      db,
    ...
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM floor_plan_table_guests WHERE guest_id=$1`,
          [g],
        )
      ).rows[0]!.c,
    ).toBe(2);
  ...
  it('seat_number > 100 rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO floor_plan_table_guests (table_id, guest_id, tenant_id, event_id, seat_number) VALUES ($1,$2,$3,$4, 200)`,
      [tb, g, t, e],
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      ...
      'anon',
      async () =>
        (await db.query<{ guest_id: string }>(`SELECT guest_id FROM floor_plan_table_guests`)).rows
          .length,
    );
    expect(anon).toBe(0);
