...
describe('RLS on event_types (Phase 12 Unit 70)', () => {
  ...
  it('system rows are visible to any authenticated tenant_member', async () => {
    await db.query(
      `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES (NULL, 'wedding', 'Wedding', TRUE)`,
    );
    ...
    const n = (await db.query<{ id: string }>(`SELECT id FROM event_types WHERE tenant_id IS NULL`))
      .rows.length;
  ...
  it('system rows are NOT visible to anon', async () => {
    ...
    const n = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM event_types`)).rows.length,
    );
    expect(n).toBe(0);
  });
  ...
  it('tenant rows are scoped to own tenant', async () => {
    ...
    await db.query(
      `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES ($1, 'custom-1', 'Custom 1', FALSE)`,
      [t1],
    );
    await db.query(
      `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES ($1, 'custom-2', 'Custom 2', FALSE)`,
      [t2],
    );
    ...
    await asRole(db, 'authenticated');
    const codes = (
      await db.query<{ code: string }>(`SELECT code FROM event_types WHERE tenant_id IS NOT NULL`)
    ).rows.map((r) => r.code);
    await asSuperuser(db);
  ...
  it('owner can INSERT a tenant-scoped event_type', async () => {
    ...
    await asRole(db, 'authenticated');
    await db.query(
      `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES ($1, 'gala', 'Gala', FALSE)`,
      [t],
    );
    await asSuperuser(db);
    const c = (
      await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_types WHERE code='gala'`)
    ).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('tenant_member cannot INSERT a system event_type', async () => {
    ...
    try {
      await db.query(
        `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES (NULL, 'sneak', 'Sneak', TRUE)`,
      );
    } catch (e) {
  ...
  it('super_admin can INSERT a system event_type', async () => {
    ...
    await asRole(db, 'authenticated');
    await db.query(
      `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES (NULL, 'platform-evt', 'Platform Event', TRUE)`,
    );
    await asSuperuser(db);
    const c = (
      await db.query<{ c: number }>(
        `SELECT count(*)::int AS c FROM event_types WHERE code='platform-evt'`,
      )
    ).rows[0]!.c;
    expect(c).toBe(1);
  });
