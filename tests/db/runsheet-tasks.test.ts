...
async function mkSubteam(db: TestDb, tenant: string, event: string, name='S'): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1,$2,$3) RETURNING id`, [tenant, event, name])).rows[0]!.id;
}
async function mkTask(db: TestDb, t: string, e: string, title='T', overrides: Record<string, unknown> = {}): Promise<string> {
  const cols = ['tenant_id','event_id','title', ...Object.keys(overrides)];
  const vals = [t, e, title, ...Object.values(overrides)];
  const placeholders = vals.map((_, i) => `$${i+1}`).join(',');
  return (await db.query<{ id: string }>(
    `INSERT INTO runsheet_tasks (${cols.join(',')}) VALUES (${placeholders}) RETURNING id`, vals)).rows[0]!.id;
}
...
describe('runsheet_tasks — schema correctness (Phase 3 Unit 26)', () => {
  ...
  it('inserts a valid pending task', async () => {
    ...
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM runsheet_tasks`)).rows[0]!.c).toBe(1);
  });
  ...
  it('rejects bad status', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, status) VALUES ($1,$2,'T','spinning')`, [t, e]);
  ...
  it('rejects bad priority', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, priority) VALUES ($1,$2,'T','ULTRA')`, [t, e]);
  ...
  it('in_progress requires actual_start', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, status) VALUES ($1,$2,'T','in_progress')`, [t, e]);
  ...
  it('completed requires both actual_* timestamps', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, status, actual_start) VALUES ($1,$2,'T','completed', now())`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('blocked requires blocked_reason', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, status) VALUES ($1,$2,'T','blocked')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('no_self_dependency blocks task pointing at itself', async () => {
    ...
    const err = await tryExec(db, `UPDATE runsheet_tasks SET depends_on_id = id WHERE id = $1`, [task]);
  ...
  it('cycle: A->B, B->A rejected', async () => {
    ...
    const err = await tryExec(db, `UPDATE runsheet_tasks SET depends_on_id = $1 WHERE id = $2`, [b, a]);
  ...
  it('cross-tenant assignee rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, assigned_to) VALUES ($1,$2,'T',$3)`, [t1, e1, mOther]);
  ...
  it('dependency from another event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, depends_on_id) VALUES ($1,$2,'T',$3)`, [t, e1, taskOnE2]);
  ...
  it('subteam from wrong event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO runsheet_tasks (tenant_id, event_id, title, subteam_id) VALUES ($1,$2,'T',$3)`, [t, e1, stOnE2]);
