...
describe('guests — schema correctness (Phase 3 Unit 16)', () => {
  ...
  it('inserts a valid guest', async () => {
    ...
    await db.query(
      `INSERT INTO guests (tenant_id, event_id, name, email, phone, category)
       VALUES ($1,$2,'Alice','alice@y.dev','+919876543210','VIP')`, [t, e]);
  ...
  it('partial UNIQUE: same email twice in same event rejected (citext fold)', async () => {
    ...
    const e = await mkEvent(db, t);
    await db.query(`INSERT INTO guests (tenant_id, event_id, name, email) VALUES ($1,$2,'A','a@y.dev')`, [t, e]);
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, email) VALUES ($1,$2,'A2','A@Y.DEV')`, [t, e]);
  ...
  it('partial UNIQUE: same email in TWO events allowed', async () => {
    ...
    await db.query(`INSERT INTO guests (tenant_id, event_id, name, email) VALUES ($1,$2,'A','x@y.dev')`, [t, e1]);
    await db.query(`INSERT INTO guests (tenant_id, event_id, name, email) VALUES ($1,$2,'A','x@y.dev')`, [t, e2]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM guests WHERE email='x@y.dev'`)).rows[0]!.c).toBe(2);
  });
  ...
  it('rejects rsvp_responded_at while pending', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, rsvp_status, rsvp_responded_at) VALUES ($1,$2,'A','pending', now())`, [t, e]);
  ...
  it('approved requires approver + approved_at', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, registration_status) VALUES ($1,$2,'A','approved')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('rejected requires rejection_reason', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, registration_status) VALUES ($1,$2,'A','rejected')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('checked_in requires check_in_at', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, check_in_status) VALUES ($1,$2,'A','checked_in')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('GDPR erased: PII columns must be NULL when erased_at set', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name, email, erased_at, erased_reason)
       VALUES ($1,$2,'[Erased]','still@here.com', now(), 'gdpr_request')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('GDPR erased: NULL PII + erased_reason accepted', async () => {
    ...
    await db.query(
      `INSERT INTO guests (tenant_id, event_id, name, erased_at, erased_reason)
       VALUES ($1,$2,'[Erased]', now(), 'gdpr_request')`, [t, e]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM guests WHERE erased_at IS NOT NULL`)).rows[0]!.c).toBe(1);
  });
  ...
  it('cross-tenant attack: guests.tenant_id != events.tenant_id rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guests (tenant_id, event_id, name) VALUES ($1,$2,'Mallory')`, [t2, e1]);
