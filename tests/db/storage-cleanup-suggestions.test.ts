...
async function mkSug(
  db: TestDb,
  tenant: string,
  desc = 'Sweep old exports',
  stype = 'delete_old_exports',
): Promise<string> {
  return (
    await db.query<{ id: string }>(
...
describe('storage_cleanup_suggestions — schema correctness (Phase 7 Unit 50)', () => {
  ...
  it('inserts a valid suggestion', async () => {
    ...
    await db.query(
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, bytes_to_free, target_object_count, priority)
       VALUES ($1,'archive_old_events','Archive events older than 1 year', 5368709120, 1200, 'high')`,
      [t],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM storage_cleanup_suggestions`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('rejects bad suggestion_type', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description) VALUES ($1,'sell_data','Sell tenant data')`,
      [t],
    );
  ...
  it('rejects bad priority', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, priority) VALUES ($1,'delete_duplicates','X','ultra')`,
      [t],
    );
  ...
  it('dismissed requires reason', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, status, dismissed_at, dismissed_by)
       VALUES ($1,'delete_old_exports','X','dismissed', now(), $2)`,
      [t, m],
    );
  ...
  it('applied requires applied_at', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, status)
       VALUES ($1,'delete_old_exports','X','applied')`,
      [t],
    );
    expect(err).toMatch(/check/i);
  });
  ...
  it('superseded requires superseded_by', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, status)
       VALUES ($1,'delete_old_exports','X','superseded')`,
      [t],
    );
    expect(err).toMatch(/check/i);
  });
  ...
  it('supersede cycle A->B, B->A rejected', async () => {
    ...
    const b = (
      await db.query<{ id: string }>(
        `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, superseded_by) VALUES ($1,'delete_old_exports','B',$2) RETURNING id`,
        [t, a],
      )
    ).rows[0]!.id;
    const err = await tryExec(
      db,
      `UPDATE storage_cleanup_suggestions SET superseded_by = $1 WHERE id = $2`,
      [b, a],
    );
  ...
  it('no_self_supersede blocks self-reference', async () => {
    ...
    const err = await tryExec(
      db,
      `UPDATE storage_cleanup_suggestions SET superseded_by = id WHERE id = $1`,
      [a],
    );
  ...
  it('expires_at must be after generated_at', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, generated_at, expires_at)
       VALUES ($1,'delete_old_exports','X', now(), now() - interval '1 hour')`,
      [t],
    );
    expect(err).toMatch(/check/i);
  });
  ...
  it('cross-tenant applier rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_cleanup_suggestions (tenant_id, suggestion_type, description, status, applied_at, applied_by)
       VALUES ($1,'delete_old_exports','X','applied', now(), $2)`,
      [t1, mOther],
    );
