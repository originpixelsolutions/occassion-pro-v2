...
async function mkSubteam(db: TestDb, tenantId: string, code: string): Promise<{ subteamId: string }> {
  ...
  const sid = (await db.query<{ id: string }>(
    `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'Decor') RETURNING id`, [tenantId, eid])).rows[0]!.id;
...
describe('RLS on event_subteam_members (Phase 12 Unit 72)', () => {
  ...
  it('member sees own tenant subteam members only', async () => {
    ...
    await db.query(`INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [s1, u1]);
    await db.query(`INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [s2, u2]);
    await setCtx(db, u1, 'tenant_member', t1);
    await asRole(db, 'authenticated');
    const n = (await db.query<{ subteam_id: string }>(`SELECT subteam_id FROM event_subteam_members`)).rows.length;
  ...
  it('anon sees zero', async () => {
    ...
    await db.query(`INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [subteamId, u]);
  ...
  it('team_member cannot INSERT', async () => {
    ...
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [subteamId, u]); }
