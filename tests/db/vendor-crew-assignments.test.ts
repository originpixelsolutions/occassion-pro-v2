...
async function mkCrew(db: TestDb, vendor: string, name='Ravi'): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO vendor_crew_members (vendor_account_id, full_name) VALUES ($1,$2) RETURNING id`, [vendor, name])).rows[0]!.id;
}
...
describe('vendor_crew_assignments — schema correctness (Phase 3 Unit 40)', () => {
  ...
  it('inserts a valid scheduled assignment', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id, role_on_event, shift_start, shift_end)
       VALUES ($1,$2,$3,$4,$5,'Lead Server', now()+interval '1 day', now()+interval '1 day 8 hours')`, [a, m, v, t, e]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_crew_assignments`)).rows[0]!.c).toBe(1);
  });
  ...
  it('UNIQUE (assignment, crew) blocks dupe', async () => {
    ...
    const a = await mkAssignment(db, t, e, v);
    const m = await mkCrew(db, v);
    await db.query(`INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id) VALUES ($1,$2,$3,$4,$5)`, [a, m, v, t, e]);
    const err = await tryExec(db, `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id) VALUES ($1,$2,$3,$4,$5)`, [a, m, v, t, e]);
  ...
  it('rejects shift_end <= shift_start', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id, shift_start, shift_end)
       VALUES ($1,$2,$3,$4,$5, now()+interval '2 hours', now()+interval '1 hour')`, [a, m, v, t, e]);
  ...
  it('checked_in requires checked_in_at', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id, status)
       VALUES ($1,$2,$3,$4,$5,'checked_in')`, [a, m, v, t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('checked_out requires both timestamps and ordering', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id, status, checked_in_at, checked_out_at)
       VALUES ($1,$2,$3,$4,$5,'checked_out', now(), now() - interval '1 hour')`, [a, m, v, t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('cancelled requires cancelled_at + reason', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id, status)
       VALUES ($1,$2,$3,$4,$5,'cancelled')`, [a, m, v, t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('crew from another vendor rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id) VALUES ($1,$2,$3,$4,$5)`, [a1, mOther, v1, t, e]);
  ...
  it('assignment vendor mismatch rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id) VALUES ($1,$2,$3,$4,$5)`, [a1, m2, v2, t, e]);
  ...
  it('cross-tenant event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_crew_assignments (vendor_assignment_id, crew_member_id, vendor_account_id, tenant_id, event_id) VALUES ($1,$2,$3,$4,$5)`, [a1, m, v, t2, e1]);
    expect(err).toMatch(/parents|does not match/i);
  });
