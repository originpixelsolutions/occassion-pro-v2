...
async function mkSubteam(db: TestDb, tenant: string, event: string, name = 'Catering'): Promise<string> {
  const r = await db.query<{ id: string }>(
...
describe('event_subteam_members — schema correctness (Phase 3 Unit 3)', () => {
  ...
  it('inserts a valid assignment', async () => {
    ...
    await db.query(
      `INSERT INTO event_subteam_members (subteam_id, member_id, role_label)
       VALUES ($1, $2, 'cook')`, [s, m]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_subteam_members`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('composite PK blocks duplicate (subteam, member)', async () => {
    ...
    const m = await mkMember(db, t, 'a@y.dev');
    await db.query(
      `INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [s, m]);
    const err = await tryExec(db,
      `INSERT INTO event_subteam_members (subteam_id, member_id, role_label) VALUES ($1, $2, 'cook')`, [s, m]);
  ...
  it('rejects empty/whitespace role_label', async () => {
    ...
    const m = await mkMember(db, t, 'a@y.dev');
    const err = await tryExec(db,
      `INSERT INTO event_subteam_members (subteam_id, member_id, role_label) VALUES ($1, $2, '   ')`, [s, m]);
  ...
  it('member CAN belong to multiple subteams in same event', async () => {
    ...
    const m = await mkMember(db, t, 'a@y.dev');
    await db.query(
      `INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $3), ($2, $3)`, [s1, s2, m]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_subteam_members`)).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('trigger: rejects cross-tenant member assignment', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_subteam_members (subteam_id, member_id) VALUES ($1, $2)`, [s, m_other_tenant]);
  ...
  it('SET NULL: deleting added_by nulls it but keeps the assignment', async () => {
    ...
    await db.query(
      `INSERT INTO event_subteam_members (subteam_id, member_id, added_by) VALUES ($1, $2, $3)`, [s, m, adder]);
    await db.query(`DELETE FROM tenant_members WHERE id = $1`, [adder]);
    const r = await db.query<{ added_by: string | null }>(`SELECT added_by FROM event_subteam_members`);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ subteam_id: string }>(`SELECT subteam_id FROM event_subteam_members`)).rows.length);
    expect(anon).toBe(0);
