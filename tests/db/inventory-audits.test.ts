...
describe('inventory_audits — schema correctness (Phase 3 Unit 31)', () => {
  ...
  it('inserts a valid in_progress audit', async () => {
    ...
    await db.query(
      `INSERT INTO inventory_audits (tenant_id, audited_by, audit_type) VALUES ($1,$2,'periodic')`, [t, m]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM inventory_audits`)).rows[0]!.c).toBe(1);
  });

  it('rejects bad audit_type', async () => {
    const t = await mkTenant(db, 'iau-bbb');
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, audit_type) VALUES ($1,'manual_count')`, [t]);
  ...
  it('completed requires completed_at + snapshot', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, status) VALUES ($1,'completed')`, [t]);
  ...
  it('cancelled requires cancelled_at + reason', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, status) VALUES ($1,'cancelled')`, [t]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('completed happy path', async () => {
    ...
    await db.query(
      `INSERT INTO inventory_audits (tenant_id, status, snapshot, completed_at, item_count, discrepancy_count)
       VALUES ($1,'completed', '{"items":[]}'::jsonb, now(), 42, 0)`, [t]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM inventory_audits WHERE status='completed'`)).rows[0]!.c).toBe(1);
  });

  it('rejects array snapshot', async () => {
    const t = await mkTenant(db, 'iau-fff');
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, snapshot) VALUES ($1, '[]'::jsonb)`, [t]);
  ...
  it('value without currency rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, total_value_audited) VALUES ($1, 250000.00)`, [t]);
  ...
  it('completed_at before audited_at rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, audited_at, status, snapshot, completed_at)
       VALUES ($1, now(), 'completed', '{}'::jsonb, now() - interval '1 hour')`, [t]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('cross-tenant auditor rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO inventory_audits (tenant_id, audited_by) VALUES ($1,$2)`, [t1, mOther]);
  ...
  it('partial index: only audits with discrepancies > 0 indexed', async () => {
    ...
    await db.query(
      `INSERT INTO inventory_audits (tenant_id, discrepancy_count) VALUES ($1, 0)`, [t]);
    await db.query(
      `INSERT INTO inventory_audits (tenant_id, discrepancy_count) VALUES ($1, 3)`, [t]);
    const discrepant = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM inventory_audits WHERE discrepancy_count > 0`)).rows[0]!.c;
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM inventory_audits`)).rows.length);
    expect(anon).toBe(0);
