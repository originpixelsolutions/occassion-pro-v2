...
describe('RLS on event_subteams (Phase 12 Unit 71)', () => {
  ...
  it('member sees own tenant subteams only', async () => {
    ...
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'Decor')`,
      [t1, e1],
    );
    await db.query(
    ...
    const names = (await db.query<{ name: string }>(`SELECT name FROM event_subteams`)).rows.map(
      (r) => r.name,
    );
  ...
  it('owner can INSERT subteams', async () => {
    ...
    await asRole(db, 'authenticated');
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'New')`,
      [t, e],
    );
    await asSuperuser(db);
    const c = (
      await db.query<{ c: number }>(
        `SELECT count(*)::int AS c FROM event_subteams WHERE name='New'`,
      )
    ).rows[0]!.c;
    expect(c).toBe(1);
  });
