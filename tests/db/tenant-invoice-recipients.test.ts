...
describe('tenant_invoice_recipients — schema correctness (Phase 2 Unit 13)', () => {
  ...
  it('inserts a valid recipient', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_invoice_recipients (tenant_id, email, name, role)
       VALUES ($1, 'finance@acme.co', 'Finance Team', 'finance')`, [t]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_invoice_recipients`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects malformed email', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_invoice_recipients (tenant_id, email) VALUES ($1, 'not-an-email')`, [t]);
  ...
  it('rejects bogus role', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO tenant_invoice_recipients (tenant_id, email, role) VALUES ($1, 'a@y.dev', 'monarch')`, [t]);
  ...
  it('rejects all-channels-off', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_invoice_recipients (tenant_id, email, receive_invoices, receive_receipts, receive_dunning)
       VALUES ($1, 'a@y.dev', FALSE, FALSE, FALSE)`, [t]);
  ...
  it('partial UNIQUE: blocks duplicate active email per tenant', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO tenant_invoice_recipients (tenant_id, email) VALUES ($1, 'a@y.dev')`, [t]);
    const err = await tryExec(db,
      `INSERT INTO tenant_invoice_recipients (tenant_id, email) VALUES ($1, 'a@y.dev')`, [t]);
  ...
  it('a removed recipient does not block a new one with same email', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO tenant_invoice_recipients (tenant_id, email, removed_at) VALUES ($1, 'a@y.dev', now())`, [t]);
    await db.query(
      `INSERT INTO tenant_invoice_recipients (tenant_id, email) VALUES ($1, 'a@y.dev')`, [t]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_invoice_recipients`)).rows[0]!.c;
