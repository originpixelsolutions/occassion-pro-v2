...
describe('speaker_accounts — schema correctness (Phase 3 Unit 15)', () => {
  ...
  it('inserts a magic-link speaker (no password)', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM speaker_accounts`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects oversize bio', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO speaker_accounts (email, bio) VALUES ('s@y.dev', $1)`,
      [long],
    );
  ...
  it('mfa_secret_coupling: enabled without secret rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO speaker_accounts (email, mfa_enabled, password_hash) VALUES ('s@y.dev', TRUE, $1)`,
      [PW],
    );
  ...
  it('mfa fully enabled accepted', async () => {
    await db.query(
      `INSERT INTO speaker_accounts (email, password_hash, mfa_enabled, mfa_secret)
       VALUES ('s@y.dev', $1, TRUE, '\\x00aabb'::bytea)`,
      [PW],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM speaker_accounts`))
      .rows[0]!.c;
  ...
  it('expertise_tags GIN searchable', async () => {
    await db.query(
      `INSERT INTO speaker_accounts (email, expertise_tags) VALUES ('s@y.dev', ARRAY['ai','design'])`,
    );
    const r = await db.query<{ id: string }>(
      `SELECT id FROM speaker_accounts WHERE expertise_tags && ARRAY['ai']`,
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM speaker_accounts`)).rows.length,
    );
    expect(anon).toBe(0);
