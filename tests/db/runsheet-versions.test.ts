...
async function mkFull(db: TestDb, t: string, e: string, label = 'v0'): Promise<string> {
  return (
    await db.query<{ id: string }>(
     ...
     VALUES ($1,$2, TRUE, '{"tasks":[]}'::jsonb, $3) RETURNING id`,
      [t, e, label],
    )
  ).rows[0]!.id;
}
...
describe('runsheet_versions — schema correctness (Phase 3 Unit 28)', () => {
    ...
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM runsheet_versions WHERE is_full=TRUE`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('inserts a valid DIFF on top of a FULL', async () => {
    ...
    await db.query(
      `INSERT INTO runsheet_versions (tenant_id, event_id, is_full, diff, base_version_id)
       VALUES ($1,$2, FALSE, '{"ops":[{"add":"task1"}]}'::jsonb, $3)`,
      [t, e, base],
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM runsheet_versions WHERE is_full=FALSE`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('FULL without snapshot rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO runsheet_versions (tenant_id, event_id, is_full) VALUES ($1,$2, TRUE)`,
      [t, e],
    );
    expect(err).toMatch(/full_or_diff|check/i);
  });
  ...
  it('cycle: v1 base=v2, v2 base=v1 rejected', async () => {
    ...
    const b = (
      await db.query<{ id: string }>(
        `INSERT INTO runsheet_versions (tenant_id, event_id, is_full, diff, base_version_id)
       VALUES ($1,$2, FALSE, '{}'::jsonb, $3) RETURNING id`,
        [t, e, a],
      )
    ).rows[0]!.id;
    const err = await tryExec(
      db,
      `UPDATE runsheet_versions SET base_version_id = $1 WHERE id = $2`,
      [b, a],
    );
