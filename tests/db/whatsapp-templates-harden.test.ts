...
describe('whatsapp_templates — hardened correctness (Phase 8 Unit 51a)', () => {
  ...
  it('inserts a valid pending template', async () => {
    ...
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM whatsapp_templates`)).rows[0]!.c).toBe(1);
  });
  ...
  it('accepts en_US / en_GB language codes', async () => {
    await db.query(`INSERT INTO whatsapp_templates (template_name, category, language_code, body_text) VALUES ('greet_us','transactional','en_US','Hi')`);
    await db.query(`INSERT INTO whatsapp_templates (template_name, category, language_code, body_text) VALUES ('greet_uk','transactional','en_GB','Hi')`);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM whatsapp_templates`)).rows[0]!.c).toBe(2);
  });
  ...
  it('rejects array buttons > 10', async () => {
    const buttons = JSON.stringify(Array.from({length: 11}, (_,i) => ({type:'quick_reply', text:'B'+i})));
    const err = await tryExec(db,
      `INSERT INTO whatsapp_templates (template_name, category, body_text, buttons) VALUES ('t1','transactional','b', $1::jsonb)`, [buttons]);
  ...
  it('approved happy path', async () => {
    ...
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM whatsapp_templates WHERE meta_status='approved'`)).rows[0]!.c).toBe(1);
  });
