...
describe('guest_plus_ones — schema correctness (Phase 3 Unit 19)', () => {
  ...
  it('rejects bad age_category', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id, age_category) VALUES ($1,$2,$3,'teen')`, [t, e, g]);
  ...
  it('rejects bad rsvp_status', async () => {
    ...
    const g = await mkGuest(db, t, e);
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id, rsvp_status) VALUES ($1,$2,$3,'tentative')`, [t, e, g]);
  ...
  it('checked_out requires both timestamps and ordering', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id, check_in_status, check_in_at, check_out_at)
       VALUES ($1,$2,$3,'checked_out', now(), now() - interval '1 hour')`, [t, e, g]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('GDPR erased: PII columns must be NULL when erased', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id, name, erased_at, erased_reason)
       VALUES ($1,$2,$3,'Still Here', now(), 'gdpr_request')`, [t, e, g]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('GDPR erased: NULL PII + reason accepted', async () => {
    ...
    await db.query(
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id, erased_at, erased_reason)
       VALUES ($1,$2,$3, now(), 'gdpr_request')`, [t, e, g]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM guest_plus_ones WHERE erased_at IS NOT NULL`)).rows[0]!.c).toBe(1);
  });
  ...
  it('cross-tenant: primary guest from another tenant rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id) VALUES ($1,$2,$3)`, [t1, e1, gOther]);
  ...
  it('primary guest belongs to wrong event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id) VALUES ($1,$2,$3)`, [t, e1, gOnE2]);
  ...
  it('RLS pair', async () => {
    ...
    const e = await mkEvent(db, t);
    const g = await mkGuest(db, t, e);
    await db.query(`INSERT INTO guest_plus_ones (tenant_id, event_id, primary_guest_id) VALUES ($1,$2,$3)`, [t, e, g]);
