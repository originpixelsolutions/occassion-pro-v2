...
describe('inventory_items — schema correctness (Phase 3 Unit 29)', () => {
  ...
  it('inserts a valid item', async () => {
    ...
    await db.query(
      `INSERT INTO inventory_items (tenant_id, name, category, sku, quantity_total, quantity_in_stock, unit_cost, currency_code)
       VALUES ($1,'Round Table 8-seater','tables','RT-8', 50, 50, 12500.00, 'INR')`,
      [t],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM inventory_items`)).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('rejects bad category', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, category) VALUES ($1,'X','spaceship')`,
      [t],
    );
  ...
  it('cost without currency rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, unit_cost) VALUES ($1,'X', 100.00)`,
      [t],
    );
  ...
  it('retired requires retired_at', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, status) VALUES ($1,'X','retired')`,
      [t],
    );
  ...
  it('partial UNIQUE sku per tenant (case-insensitive)', async () => {
    ...
    await db.query(`INSERT INTO inventory_items (tenant_id, name, sku) VALUES ($1,'A','RT-8')`, [
      t,
    ]);
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, sku) VALUES ($1,'B','rt-8')`,
      [t],
    );
    ...
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM inventory_items WHERE sku='RT-8'`,
        )
      ).rows[0]!.c,
    ).toBe(2);
  });
  ...
  it('rejects bad sku format', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, sku) VALUES ($1,'X','sku with spaces')`,
      [t],
    );
  ...
  it('rejects non-https image_url', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_items (tenant_id, name, image_url) VALUES ($1,'X','http://insecure/x.jpg')`,
      [t],
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM inventory_items`)).rows.length,
    );
    expect(anon).toBe(0);
