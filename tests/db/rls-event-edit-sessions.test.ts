...
describe('RLS on event_edit_sessions (Phase 12 Unit 82)', () => {
  ...
  it('self can INSERT own edit session', async () => {
    ...
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_edit_sessions`)).rows[0]!
        .c,
    ).toBe(1);
  });
  ...
  it('non-owner cannot UPDATE someone elses session', async () => {
    ...
    const r = await db.query<{ c: number }>(
      `WITH u AS (UPDATE event_edit_sessions SET released_at=now(), released_reason='hijack' WHERE id=$1 RETURNING id) SELECT count(*)::int AS c FROM u`,
      [id],
    );
    await asSuperuser(db);
  ...
  it('manager CAN force-release', async () => {
    ...
    await asRole(db, 'authenticated');
    const r = await db.query<{ c: number }>(
      `WITH d AS (DELETE FROM event_edit_sessions WHERE id=$1 RETURNING id) SELECT count(*)::int AS c FROM d`,
      [id],
    );
    await asSuperuser(db);
