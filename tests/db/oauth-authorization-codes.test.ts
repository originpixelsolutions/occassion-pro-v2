...
async function mkApp(db: TestDb, tenant: string, clientSuffix = 'aaaabbbbccccdddd'): Promise<string> {
  const r = await db.query<{ id: string }>(
...
describe('oauth_authorization_codes — schema correctness (Phase 2 Unit 42)', () => {
  ...
  it('inserts a valid code', async () => {
    ...
    await db.query(
      ...
      [app, CODE, USER]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM oauth_authorization_codes`)).rows[0]!.c;
    expect(c).toBe(1);
  });
