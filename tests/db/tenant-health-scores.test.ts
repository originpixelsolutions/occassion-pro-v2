...
describe('tenant_health_scores — schema correctness (Phase 2 Unit 39)', () => {
  ...
  it('inserts a fully-populated row', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_health_scores (
       ...
       ) VALUES ($1, 82.50, 85, 80, 90, 70, 75, 'low', ARRAY['active_user_growth'])`, [t]);
  ...
  it('PK enforces singleton per tenant', async () => {
    const t = await mkTenant(db, 'acme-co');
    await db.query(
      `INSERT INTO tenant_health_scores (tenant_id) VALUES ($1)`, [t]);
  ...
  it('rejects score outside 0..100', async () => {
    ...
    const e1 = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, overall_score, churn_risk_level) VALUES ($1, -1, 'low')`, [t]);
    expect(e1).toMatch(/overall_bounds|check/i);
    const e2 = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, product_engagement_score) VALUES ($1, 101)`, [t]);
  ...
  it('rejects overall_score set without churn_risk_level (coupling)', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, overall_score) VALUES ($1, 75)`, [t]);
  ...
  it('rejects churn_risk_level set without overall_score', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, churn_risk_level) VALUES ($1, 'low')`, [t]);
    expect(err).toMatch(/overall_risk_coupling|check/i);
  });
  ...
  it('rejects empty churn_risk_reasons array', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, overall_score, churn_risk_level, churn_risk_reasons)
       VALUES ($1, 25, 'critical', ARRAY[]::text[])`, [t]);
  ...
  it('rejects negative counters', async () => {
    const t = await mkTenant(db, 'acme-co');
    const err = await tryExec(db,
      `INSERT INTO tenant_health_scores (tenant_id, ticket_count_30d) VALUES ($1, -1)`, [t]);
  ...
  it('allows a sparse row (just tenant_id + computed_at default)', async () => {
    ...
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_health_scores`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM tenant_health_scores`)).rows.length);
    expect(anon).toBe(0);
