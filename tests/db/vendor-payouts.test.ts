...
async function mkAssignment(db: TestDb, t: string, e: string, v: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO vendor_event_assignments (vendor_account_id, tenant_id, event_id, service_category)
     VALUES ($1,$2,$3,'catering') RETURNING id`, [v, t, e])).rows[0]!.id;
}
...
describe('vendor_payouts — schema correctness (Phase 6 Unit 47)', () => {
  ...
  it('inserts a valid scheduled payout', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_payouts (tenant_id, event_id, vendor_account_id, assignment_id, milestone, milestone_type, amount, currency_code, scheduled_for, bank_account_last4, ifsc_code)
       VALUES ($1,$2,$3,$4,'50% booking advance','booking_advance', 125000, 'INR', now()+interval '3 days', '1234', 'HDFC0000123')`, [t, e, v, a]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_payouts`)).rows[0]!.c).toBe(1);
  });

  it('amount > 0 enforced', async () => {
    const t = await mkTenant(db, 'vpo-bbb');
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code) VALUES ($1,'X', 0, 'INR')`, [t]);
  ...
  it('rejects bad ifsc_code', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, ifsc_code) VALUES ($1,'X', 100, 'INR','BAD123')`, [t]);
  ...
  it('rejects bad bank_account_last4', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, bank_account_last4) VALUES ($1,'X', 100, 'INR','12')`, [t]);
  ...
  it('rejects bad gateway', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, gateway) VALUES ($1,'X', 100, 'INR','crypto')`, [t]);
  ...
  it('approved requires approver + approved_at', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, status) VALUES ($1,'X', 100, 'INR','approved')`, [t]);
  ...
  it('disbursing requires gateway selection', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, status, approved_at, approved_by)
       VALUES ($1,'X', 100, 'INR','disbursing', now(), $2)`, [t, m]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('disbursed requires approval + disbursed_at + gateway', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, status, approved_at, approved_by, disbursed_at)
       VALUES ($1,'X', 100, 'INR','disbursed', now(), $2, now())`, [t, m]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('failed requires failure_reason', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, status, failed_at)
       VALUES ($1,'X', 100, 'INR','failed', now())`, [t]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('UNIQUE (gateway, gateway_payout_id) blocks dupe', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, gateway, gateway_payout_id)
       VALUES ($1,'X', 100, 'INR','razorpay_x','pyt_R1')`, [t]);
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, gateway, gateway_payout_id)
       VALUES ($1,'Y', 100, 'INR','razorpay_x','pyt_R1')`, [t]);
  ...
  it('net_amount generated column', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code, fees) VALUES ($1,'X', 10000, 'INR', 150)`, [t]);
    const n = (await db.query<{ n: string }>(`SELECT net_amount::text AS n FROM vendor_payouts`)).rows[0]!.n;
  ...
  it('assignment vendor mismatch rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, event_id, vendor_account_id, assignment_id, milestone, amount, currency_code)
       VALUES ($1,$2,$3,$4,'X', 100, 'INR')`, [t, e, v2, a1]);
  ...
  it('cross-tenant event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO vendor_payouts (tenant_id, event_id, milestone, amount, currency_code) VALUES ($1,$2,'X', 100, 'INR')`, [t1, e2]);
    expect(err).toMatch(/tenant|does not match/i);
  });

  it('RLS pair', async () => {
    const t = await mkTenant(db, 'vpo-www');
    await db.query(`INSERT INTO vendor_payouts (tenant_id, milestone, amount, currency_code) VALUES ($1,'X', 100, 'INR')`, [t]);
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM vendor_payouts`)).rows.length);
    expect(anon).toBe(0);
