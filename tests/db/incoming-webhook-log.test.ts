...
describe('incoming_webhook_log — schema correctness (Phase 9 Unit 58)', () => {
  ...
  it('UNIQUE (source, external_id) blocks dupe', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('stripe','evt_X1','{}'::jsonb)`);
    const err = await tryExec(db, `INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('stripe','evt_X1','{}'::jsonb)`);
    expect(err).toMatch(/duplicate|unique/i);
  });

  it('same external_id across TWO sources allowed', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('razorpay','xyz123','{}'::jsonb)`);
    await db.query(`INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('stripe','xyz123','{}'::jsonb)`);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM incoming_webhook_log WHERE external_id='xyz123'`)).rows[0]!.c).toBe(2);
  });
  ...
  it('payload array accepted (Stripe batch)', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, payload) VALUES ('stripe','[{"e":"x"}]'::jsonb)`);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM incoming_webhook_log`)).rows[0]!.c).toBe(1);
  });

  it('canonical-field UPDATE blocked', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('stripe','locked','{}'::jsonb)`);
    const err = await tryExec(db, `UPDATE incoming_webhook_log SET payload = '{"tampered":true}'::jsonb WHERE external_id = 'locked'`);
    expect(err).toMatch(/immutable|append-only/i);
  });

  it('status UPDATE allowed (processing -> processed)', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, external_id, payload) VALUES ('stripe','flow','{}'::jsonb)`);
    await db.query(`UPDATE incoming_webhook_log SET status = 'processing', processing_started_at = now() WHERE external_id = 'flow'`);
    await db.query(`UPDATE incoming_webhook_log SET status = 'processed', processed_at = now() WHERE external_id = 'flow'`);
    const r = await db.query<{ status: string }>(`SELECT status FROM incoming_webhook_log WHERE external_id='flow'`);
    expect(r.rows[0]!.status).toBe('processed');
  });

  it('bigserial PK auto-increments', async () => {
    await db.query(`INSERT INTO incoming_webhook_log (source, payload) VALUES ('stripe','{}'::jsonb)`);
    await db.query(`INSERT INTO incoming_webhook_log (source, payload) VALUES ('stripe','{}'::jsonb)`);
