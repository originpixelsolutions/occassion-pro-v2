...
describe('guest_refresh_tokens — schema correctness (Phase 3 Unit 18)', () => {
  ...
  it('inserts a valid token', async () => {
    ...
    await db.query(
      `INSERT INTO guest_refresh_tokens (tenant_id, event_id, guest_id, family_id, token_hash, expires_at)
       VALUES ($1,$2,$3, gen_random_uuid(), $4, now()+interval '7 days')`,
      [t, e, g, HASH],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM guest_refresh_tokens`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('rejects expiry > 90 days', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO guest_refresh_tokens (tenant_id, event_id, guest_id, family_id, token_hash, expires_at)
       VALUES ($1,$2,$3, gen_random_uuid(), $4, now()+interval '120 days')`,
      [t, e, g, HASH],
    );
  ...
  it('rev_coupling: revoked_at without reason rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO guest_refresh_tokens (tenant_id, event_id, guest_id, family_id, token_hash, expires_at, revoked_at)
       VALUES ($1,$2,$3, gen_random_uuid(), $4, now()+interval '7 days', now())`,
      [t, e, g, HASH],
    );
  ...
  it('rotation: revoked + replaced_by accepted', async () => {
    ...
    const c = (
      await db.query<{ c: number }>(
        `SELECT count(*)::int AS c FROM guest_refresh_tokens WHERE family_id=$1`,
        [fam],
      )
    ).rows[0]!.c;
    expect(c).toBe(2);
  ...
  it('partial family-live index excludes revoked', async () => {
    ...
    const live = (
      await db.query<{ c: number }>(
        `SELECT count(*)::int AS c FROM guest_refresh_tokens WHERE family_id=$1 AND revoked_at IS NULL`,
        [fam],
      )
    ).rows[0]!.c;
    expect(live).toBe(1);
