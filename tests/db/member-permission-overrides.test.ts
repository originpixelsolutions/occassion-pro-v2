...
describe('member_permission_overrides — schema correctness (Phase 2 Unit 24)', () => {
  ...
  it('inserts an explicit grant', async () => {
    ...
    await db.query(
      `INSERT INTO member_permission_overrides (tenant_id, member_id, module, can_read, can_write)
       VALUES ($1, $2, 'budget', TRUE, TRUE)`, [t, m]);
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM member_permission_overrides`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects can_write=TRUE with can_read=FALSE (explicit incoherent)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO member_permission_overrides (tenant_id, member_id, module, can_read, can_write)
       VALUES ($1, $2, 'budget', FALSE, TRUE)`, [t, m]);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM member_permission_overrides`)).rows.length);
    expect(anon).toBe(0);
