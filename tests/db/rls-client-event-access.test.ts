...
describe('RLS on client_event_access (Phase 12 Unit 86)', () => {
  ...
  it('tenant_member sees own tenant links only', async () => {
    ...
    await db.query(
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, role) VALUES ($1, $2, $3, 'primary')`,
      [c1, t1, e1],
    );
    await db.query(
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, role) VALUES ($1, $2, $3, 'primary')`,
  ...
  it('client sees own access rows only', async () => {
    ...
    await asRole(db, 'authenticated');
    const rows = await db.query<{ client_account_id: string }>(
      `SELECT client_account_id FROM client_event_access`,
    );
    await asSuperuser(db);
