...
describe('client_event_access — schema correctness (Phase 4 Unit 41)', () => {
  ...
  it('inserts a valid access row', async () => {
    ...
    await db.query(
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, role, invited_by)
       VALUES ($1,$2,$3,'primary',$4)`, [c, t, e, m]);
  ...
  it('UNIQUE (client, event) blocks dupe', async () => {
    ...
    await db.query(`INSERT INTO client_event_access (client_account_id, tenant_id, event_id) VALUES ($1,$2,$3)`, [c, t, e]);
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id) VALUES ($1,$2,$3)`, [c, t, e]);
  ...
  it('same client across TWO events allowed', async () => {
    ...
    await db.query(`INSERT INTO client_event_access (client_account_id, tenant_id, event_id) VALUES ($1,$2,$3)`, [c, t, e1]);
    await db.query(`INSERT INTO client_event_access (client_account_id, tenant_id, event_id) VALUES ($1,$2,$3)`, [c, t, e2]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM client_event_access WHERE client_account_id=$1`, [c])).rows[0]!.c).toBe(2);
  });
  ...
  it('rejects bad role', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, role) VALUES ($1,$2,$3,'admin')`, [c, t, e]);
  ...
  it('rejects array permissions', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, permissions) VALUES ($1,$2,$3,'[]'::jsonb)`, [c, t, e]);
  ...
  it('revoke requires revoked_by + reason', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, revoked_at) VALUES ($1,$2,$3, now())`, [c, t, e]);
  ...
  it('revoke happy path', async () => {
    ...
    await db.query(
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, revoked_at, revoked_by, revoked_reason)
       VALUES ($1,$2,$3, now(), $4, 'client left engagement')`, [c, t, e, m]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM client_event_access WHERE revoked_at IS NOT NULL`)).rows[0]!.c).toBe(1);
  });
  ...
  it('accepted before invited rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, invited_at, accepted_at)
       VALUES ($1,$2,$3, now(), now() - interval '1 hour')`, [c, t, e]);
  ...
  it('cross-tenant inviter rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id, invited_by) VALUES ($1,$2,$3,$4)`, [c, t1, e1, mOther]);
  ...
  it('cross-tenant event rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO client_event_access (client_account_id, tenant_id, event_id) VALUES ($1,$2,$3)`, [c, t1, e2]);
