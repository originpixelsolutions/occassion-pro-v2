...
describe('event_type_readiness_items — schema correctness (Phase 3 Unit 4)', () => {
  ...
  it('inserts a valid item', async () => {
    ...
    await db.query(
      `INSERT INTO event_type_readiness_items (event_type_id, label, module, weight, sort_order)
       VALUES ($1, 'Confirm venue', 'events', 5, 0)`, [ty]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_type_readiness_items`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects empty label', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_type_readiness_items (event_type_id, label) VALUES ($1, '   ')`, [ty]);
  ...
  it('rejects bogus module', async () => {
    const ty = await mkType(db);
    const err = await tryExec(db,
      `INSERT INTO event_type_readiness_items (event_type_id, label, module) VALUES ($1, 'X', 'unicorns')`, [ty]);
  ...
  it('rejects weight outside 0..100', async () => {
    const ty = await mkType(db);
    const err = await tryExec(db,
      `INSERT INTO event_type_readiness_items (event_type_id, label, weight) VALUES ($1, 'X', 101)`, [ty]);
  ...
  it('UNIQUE: case-insensitive label per event_type', async () => {
    const ty = await mkType(db);
    await db.query(
      `INSERT INTO event_type_readiness_items (event_type_id, label) VALUES ($1, 'Confirm Venue')`, [ty]);
    const err = await tryExec(db,
      `INSERT INTO event_type_readiness_items (event_type_id, label) VALUES ($1, 'confirm venue')`, [ty]);
  ...
  it('CASCADE: deleting event_type removes its readiness items', async () => {
    const ty = await mkType(db);
    await db.query(
      `INSERT INTO event_type_readiness_items (event_type_id, label) VALUES ($1, 'X')`, [ty]);
