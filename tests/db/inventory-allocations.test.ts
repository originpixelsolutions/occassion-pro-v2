...
describe('inventory_allocations — schema correctness (Phase 3 Unit 30)', () => {
  ...
  it('inserts a valid allocation', async () => {
    ...
    await db.query(
      `INSERT INTO inventory_allocations (tenant_id, inventory_item_id, event_id, quantity) VALUES ($1,$2,$3,30)`,
      [t, i, e],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM inventory_allocations`))
        .rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('quantity must be > 0', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_allocations (tenant_id, inventory_item_id, event_id, quantity) VALUES ($1,$2,$3,0)`,
      [t, i, e],
    );
  ...
  it('returned requires both dispatched_at and returned_at and ordering', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO inventory_allocations (tenant_id, inventory_item_id, event_id, quantity, status, dispatched_at, returned_at)
       VALUES ($1,$2,$3, 10, 'returned', now(), now() - interval '1 hour')`,
      [t, i, e],
    );
    expect(err).toMatch(/check/i);
  });
  ...
  it('damage_cost without currency rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO inventory_allocations (tenant_id, inventory_item_id, event_id, quantity, quantity_damaged, damage_notes, damage_cost)
       VALUES ($1,$2,$3, 10, 2, 'spilled', 500.00)`,
      [t, i, e],
    );
  ...
  it('cross-tenant: item from another tenant rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO inventory_allocations (tenant_id, inventory_item_id, event_id, quantity) VALUES ($1,$2,$3, 5)`,
      [t1, iOther, e1],
    );
    ...
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM inventory_allocations WHERE status='returned'`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
