...
async function mkTenant(db: TestDb, slug: string): Promise<string> {
  const r = await db.query<{ id: string }>(
    `INSERT INTO tenants (slug, company_name, billing_currency)
     VALUES ($1, 'Acme Co', 'INR') RETURNING id`, [slug]);
  return r.rows[0]!.id;
}
...
describe('tenant_feature_overrides — schema correctness (Phase 2 Unit 6)', () => {
  ...
  it('inserts an override and respects composite PK', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_feature_overrides (tenant_id, flag_code, enabled)
       VALUES ($1, 'beta_calendar', true)`, [t]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_feature_overrides`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('composite PK rejects a duplicate (tenant_id, flag_code)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO tenant_feature_overrides (tenant_id, flag_code, enabled)
       VALUES ($1, 'beta_calendar', false)`, [t]);
  ...
  it('allows the same flag enabled per tenant on different tenants', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_feature_overrides (tenant_id, flag_code, enabled)
       VALUES ($1, 'beta_calendar', true), ($2, 'beta_calendar', false)`, [t1, t2]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_feature_overrides`)).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('SET NULL: deleting set_by_admin nulls it but keeps the override', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_feature_overrides (tenant_id, flag_code, enabled, set_by_admin)
       VALUES ($1, 'beta_calendar', true, $2)`, [t, admin]);
    await db.query(`DELETE FROM super_admins WHERE id = $1`, [admin]);
    const r = await db.query<{ set_by_admin: string | null }>(
      `SELECT set_by_admin FROM tenant_feature_overrides WHERE tenant_id = $1`, [t]);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM tenant_feature_overrides`)).rows.length);
    expect(anon).toBe(0);
