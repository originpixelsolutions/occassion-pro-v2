...
describe('support_faqs — schema correctness (Phase 10 Unit 59)', () => {
  ...
  it('inserts a valid FAQ', async () => {
    ...
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM support_faqs`)).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('GIN index: tags searchable', async () => {
    await db.query(
      `INSERT INTO support_faqs (question_pattern, answer, tags) VALUES ('q','a', ARRAY['refund','payment'])`,
    );
    const r = await db.query<{ id: string }>(
      `SELECT id FROM support_faqs WHERE tags && ARRAY['refund']`,
    );
    ...
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM support_faqs WHERE last_reviewed_by IS NOT NULL`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM support_faqs`)).rows.length,
    );
    expect(anon).toBe(0);
