...
describe('storage_objects — schema correctness (Phase 7 Unit 48)', () => {
  ...
  it('inserts a valid object', async () => {
    ...
    await db.query(
      `INSERT INTO storage_objects (tenant_id, event_id, category, r2_key, filename, mime_type, size_bytes, uploaded_by_type)
       VALUES ($1,$2,'event_photo','tenants/abc/event_photo/uuid.jpg','wedding.jpg','image/jpeg', 524288, 'tenant_member')`,
      [t, e],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM storage_objects`)).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('UNIQUE r2_key blocks dupe', async () => {
    ...
    await db.query(
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'document','tenants/abc/doc/u1.pdf','x.pdf','application/pdf', 100)`,
      [t],
    );
    const err = await tryExec(
      db,
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'document','tenants/abc/doc/u1.pdf','y.pdf','application/pdf', 200)`,
      [t],
    );
  ...
  it('rejects bad category', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'malware','tenants/abc/x','x','image/jpeg', 100)`,
      [t],
    );
  ...
  it('rejects r2_key with spaces', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'document','tenants/has space/x.pdf','x.pdf','application/pdf', 100)`,
      [t],
    );
  ...
  it('rejects bad mime_type', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'document','tenants/abc/x.pdf','x.pdf','not-a-mime', 100)`,
      [t],
    );
  ...
  it('rejects size_bytes > 10 GiB', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'event_video','tenants/abc/v.mp4','v.mp4','video/mp4', 21474836480)`,
      [t],
    );
  ...
  it('archive coupling: archived_at without destination rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes, archived_at)
       VALUES ($1,'document','tenants/abc/x.pdf','x.pdf','application/pdf', 100, now())`,
      [t],
    );
  ...
  it('archive coupling: full archive accepted', async () => {
    ...
    await db.query(
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes, archived_at, archive_destination, archive_expires_at, storage_class)
       VALUES ($1,'event_photo','tenants/abc/p.jpg','p.jpg','image/jpeg', 100, now(), 's3_glacier', now()+interval '90 days', 'archive')`,
      [t],
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM storage_objects WHERE archived_at IS NOT NULL`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('rejects bad sha256 hash', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes, content_hash_sha256)
       VALUES ($1,'document','tenants/abc/x.pdf','x.pdf','application/pdf', 100, 'NOT-A-HASH')`,
      [t],
    );
  ...
  it('cross-tenant event rejected', async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO storage_objects (tenant_id, event_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,$2,'document','tenants/abc/x.pdf','x.pdf','application/pdf', 100)`,
      [t1, e2],
    );
  ...
  it('RLS pair', async () => {
    ...
    await db.query(
      `INSERT INTO storage_objects (tenant_id, category, r2_key, filename, mime_type, size_bytes)
       VALUES ($1,'document','tenants/abc/x.pdf','x.pdf','application/pdf', 100)`,
      [t],
    );
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM storage_objects`)).rows.length,
    );
    expect(anon).toBe(0);
