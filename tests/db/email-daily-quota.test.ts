...
describe('email_daily_quota — schema correctness (Phase 2 Unit 29)', () => {
  ...
  it('rejects sent_count > limit_value', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO email_daily_quota (tenant_id, date, sent_count, limit_value)
       VALUES ($1, current_date, 6000, 5000)`,
      [t],
    );
  ...
  it('atomic increment: UPDATE ... SET sent_count = sent_count + 1 honors CHECK', async () => {
    ...
    await db.query(
      `INSERT INTO email_daily_quota (tenant_id, date, sent_count, limit_value)
       VALUES ($1, current_date, 4999, 5000)`,
      [t],
    );
    // First +1 OK
    await db.query(
      `UPDATE email_daily_quota SET sent_count = sent_count + 1 WHERE tenant_id = $1`,
      [t],
    );
  ...
  it('different tenants and different days coexist', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM email_daily_quota`))
      .rows[0]!.c;
    expect(c).toBe(3);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      ...
      'anon',
      async () =>
        (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM email_daily_quota`)).rows
          .length,
    );
    expect(anon).toBe(0);
