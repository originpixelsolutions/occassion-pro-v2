...
describe('RLS on event_offload_jobs (Phase 12 Unit 83)', () => {
  ...
  it('owner can enqueue an offload job', async () => {
    ...
    await db.query(`INSERT INTO event_offload_jobs (tenant_id, event_id, status, attempt_count) VALUES ($1, $2, 'queued', 0)`, [t, e]);
    await asSuperuser(db);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_offload_jobs`)).rows[0]!.c).toBe(1);
  });
  ...
  it('team_member cannot enqueue', async () => {
    ...
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO event_offload_jobs (tenant_id, event_id, status, attempt_count) VALUES ($1, $2, 'queued', 0)`, [t, e]); }
  ...
  it('tenant_member cannot UPDATE status (worker-only)', async () => {
    ...
    const id = (await db.query<{ id: string }>(
      `INSERT INTO event_offload_jobs (tenant_id, event_id, status, attempt_count) VALUES ($1, $2, 'queued', 0) RETURNING id`, [t, e])).rows[0]!.id;
    const u = '00000000-0000-0000-0000-000000001630';
    await mkMember(db, t, u, 'o@y.dev', 'owner');
    await setCtx(db, u, 'tenant_member', t);
    await asRole(db, 'authenticated');
    const r = await db.query<{ c: number }>(
      `WITH u AS (UPDATE event_offload_jobs SET status='completed' WHERE id=$1 RETURNING id) SELECT count(*)::int AS c FROM u`, [id]);
    await asSuperuser(db);
