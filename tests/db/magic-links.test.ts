...
describe('magic_links — schema correctness (Phase 2 Unit 31)', () => {
  ...
  it('inserts a valid link', async () => {
    await db.query(
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'a@y.dev', $2, now() + interval '15 minutes')`, [USER, HASH]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM magic_links`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects bogus user_type', async () => {
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'guest', 'a@y.dev', $2, now() + interval '15 minutes')`, [USER, HASH]);
  ...
  it('rejects malformed email', async () => {
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'not-an-email', $2, now() + interval '15 minutes')`, [USER, HASH]);
  ...
  it('rejects wrong-length token_hash', async () => {
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'a@y.dev', 'short', now() + interval '15 minutes')`, [USER]);
  ...
  it('rejects expires_at past 1-hour ceiling', async () => {
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'a@y.dev', $2, now() + interval '2 hours')`, [USER, HASH]);
  ...
  it('UNIQUE on token_hash blocks duplicates', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'client', 'a@y.dev', $2, now() + interval '15 minutes')`, [USER, HASH]);
  ...
  it('partial UNIQUE: one open link per (user, user_type)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'a@y.dev', $2, now() + interval '15 minutes')`, [USER, HASH2]);
    expect(err).toMatch(/duplicate|unique/i);
  });
  ...
  it('consumed link does not block a new open link for same (user, user_type)', async () => {
    await db.query(
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at, consumed_at, consumed_ip)
       VALUES ($1, 'tenant_member', 'a@y.dev', $2, now() + interval '15 minutes', now(), '1.2.3.4'::inet)`, [USER, HASH]);
    await db.query(
      `INSERT INTO magic_links (user_id, user_type, email, token_hash, expires_at)
       VALUES ($1, 'tenant_member', 'a@y.dev', $2, now() + interval '15 minutes')`, [USER, HASH2]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM magic_links`)).rows[0]!.c;
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM magic_links`)).rows.length);
    expect(anon).toBe(0);
