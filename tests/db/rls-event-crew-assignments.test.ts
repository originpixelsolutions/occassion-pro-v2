import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { asRole, asSuperuser, setupTestDb, withRole, type TestDb } from '../setup/pg.js';

async function mkTenant(db: TestDb, slug: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO tenants (slug, company_name, billing_currency) VALUES ($1,'Acme','INR') RETURNING id`, [slug])).rows[0]!.id;
}
async function mkMember(db: TestDb, tenantId: string, uid: string, email: string, role = 'owner'): Promise<void> {
  await db.query(
    `INSERT INTO tenant_members (id, tenant_id, email, full_name, role) VALUES ($1, $2, $3, 'M', $4)`,
    [uid, tenantId, email, role]);
}
async function mkEvent(db: TestDb, tenantId: string, code: string): Promise<string> {
  const etId = (await db.query<{ id: string }>(
    `INSERT INTO event_types (tenant_id, code, name, is_system) VALUES ($1, 'w-' || $2, 'W', FALSE) RETURNING id`,
    [tenantId, code])).rows[0]!.id;
  return (await db.query<{ id: string }>(
    `INSERT INTO events (tenant_id, event_type_id, code, name, start_date, end_date, currency_code)
     VALUES ($1, $2, $3, 'E', '2026-12-01', '2026-12-03', 'INR') RETURNING id`, [tenantId, etId, code])).rows[0]!.id;
}
async function mkCrew(db: TestDb, tenantId: string, name: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO crew_pool (tenant_id, full_name, is_freelance, is_active, total_events_worked) VALUES ($1, $2, FALSE, TRUE, 0) RETURNING id`,
    [tenantId, name])).rows[0]!.id;
}
async function setCtx(db: TestDb, uid: string | null, userType: string | null, tenantId: string | null) {
  await db.query(`SELECT set_config('app.user_id', $1, false)`, [uid ?? '']);
  await db.query(`SELECT set_config('app.user_type', $1, false)`, [userType ?? '']);
  await db.query(`SELECT set_config('app.tenant_id', $1, false)`, [tenantId ?? '']);
}

describe('RLS on event_crew_assignments (Phase 12 Unit 81)', () => {
  let db: TestDb;
  beforeEach(async () => { db = await setupTestDb(); });
  afterEach(async () => { await db.close(); });

  it('member sees own tenant assignments only', async () => {
    const t1 = await mkTenant(db, 'eca-aaa');
    const t2 = await mkTenant(db, 'eca-bbb');
    const e1 = await mkEvent(db, t1, 'e-a');
    const e2 = await mkEvent(db, t2, 'e-b');
    const c1 = await mkCrew(db, t1, 'A');
    const c2 = await mkCrew(db, t2, 'B');
    await db.query(`INSERT INTO event_crew_assignments (tenant_id, event_id, crew_id, shift_start, shift_end, status) VALUES ($1, $2, $3, '2026-12-01 09:00', '2026-12-01 18:00', 'scheduled')`, [t1, e1, c1]);
    await db.query(`INSERT INTO event_crew_assignments (tenant_id, event_id, crew_id, shift_start, shift_end, status) VALUES ($1, $2, $3, '2026-12-01 09:00', '2026-12-01 18:00', 'scheduled')`, [t2, e2, c2]);
    const u = '00000000-0000-0000-0000-000000001400';
    await mkMember(db, t1, u, 'm@y.dev');
    await setCtx(db, u, 'tenant_member', t1);
    await asRole(db, 'authenticated');
    const n = (await db.query<{ id: string }>(`SELECT id FROM event_crew_assignments`)).rows.length;
    await asSuperuser(db);
    expect(n).toBe(1);
  });

  it('anon sees zero', async () => {
    const t = await mkTenant(db, 'eca-ccc');
    const e = await mkEvent(db, t, 'e-c');
    const c = await mkCrew(db, t, 'C');
    await db.query(`INSERT INTO event_crew_assignments (tenant_id, event_id, crew_id, shift_start, shift_end, status) VALUES ($1, $2, $3, '2026-12-01 09:00', '2026-12-01 18:00', 'scheduled')`, [t, e, c]);
    const n = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM event_crew_assignments`)).rows.length);
    expect(n).toBe(0);
  });

  it('owner can INSERT', async () => {
    const t = await mkTenant(db, 'eca-ddd');
    const e = await mkEvent(db, t, 'e-d');
    const c = await mkCrew(db, t, 'D');
    const u = '00000000-0000-0000-0000-000000001410';
    await mkMember(db, t, u, 'o@y.dev', 'owner');
    await setCtx(db, u, 'tenant_member', t);
    await asRole(db, 'authenticated');
    await db.query(`INSERT INTO event_crew_assignments (tenant_id, event_id, crew_id, shift_start, shift_end, status) VALUES ($1, $2, $3, '2026-12-01 09:00', '2026-12-01 18:00', 'scheduled')`, [t, e, c]);
    await asSuperuser(db);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_crew_assignments`)).rows[0]!.c).toBe(1);
  });

  it('team_member cannot INSERT', async () => {
    const t = await mkTenant(db, 'eca-eee');
    const e = await mkEvent(db, t, 'e-e');
    const c = await mkCrew(db, t, 'E');
    const u = '00000000-0000-0000-0000-000000001420';
    await mkMember(db, t, u, 'tm@y.dev', 'team_member');
    await setCtx(db, u, 'tenant_member', t);
    await asRole(db, 'authenticated');
    let err = '';
    try { await db.query(`INSERT INTO event_crew_assignments (tenant_id, event_id, crew_id, shift_start, shift_end, status) VALUES ($1, $2, $3, '2026-12-01 09:00', '2026-12-01 18:00', 'scheduled')`, [t, e, c]); }
    catch (e2) { err = e2 instanceof Error ? e2.message : String(e2); }
    await asSuperuser(db);
    expect(err).toMatch(/row-level security|policy/i);
  });
});
