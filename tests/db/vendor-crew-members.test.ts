...
describe('vendor_crew_members — schema correctness (Phase 3 Unit 39)', () => {
  ...
  it('inserts a valid crew member', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, role, phone, email, hourly_rate, currency_code)
       VALUES ($1,'Ravi Kumar','head_chef','+919876543210','ravi@kitchen.in', 500.00, 'INR')`,
      [v],
    );
    expect(
      (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM vendor_crew_members`)).rows[0]!
        .c,
    ).toBe(1);
  });
  ...
  it('rejects bad role', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, role) VALUES ($1,'A','barista')`,
      [v],
    );
  ...
  it('rejects bad phone (no +)', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, phone) VALUES ($1,'A','9876543210')`,
      [v],
    );
  ...
  it('rejects bad email format', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, email) VALUES ($1,'A','not-an-email')`,
      [v],
    );
  ...
  it('citext email case-fold', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, email) VALUES ($1,'A','Ravi@Y.DEV')`,
      [v],
    );
  ...
  it('rate without currency rejected', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, hourly_rate) VALUES ($1,'A', 500.00)`,
      [v],
    );
  ...
  it('inactive requires deactivated_at', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, status) VALUES ($1,'A','inactive')`,
      [v],
    );
  ...
  it('inactive happy path', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, status, deactivated_at, deactivated_reason)
       VALUES ($1,'A','inactive', now(), 'left the team')`,
      [v],
    );
    expect(
      (
        await db.query<{ c: number }>(
          `SELECT count(*)::int AS c FROM vendor_crew_members WHERE status='inactive'`,
        )
      ).rows[0]!.c,
    ).toBe(1);
  });
  ...
  it('rejects negative hourly_rate', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name, hourly_rate, currency_code) VALUES ($1,'A', -1, 'INR')`,
      [v],
    );
  ...
  it('two crew with same name on same vendor allowed (no UNIQUE)', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name) VALUES ($1,'John Smith')`,
      [v],
    );
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name) VALUES ($1,'John Smith')`,
  ...
  it('RLS pair', async () => {
    ...
    await db.query(
      `INSERT INTO vendor_crew_members (vendor_account_id, full_name) VALUES ($1,'A')`,
      [v],
    );
