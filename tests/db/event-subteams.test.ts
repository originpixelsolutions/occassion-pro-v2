...
async function mkEvent(db: TestDb, tenant: string, code = 'evt'): Promise<string> {
  ...
  const r = await db.query<{ id: string }>(
    `INSERT INTO events (tenant_id, event_type_id, code, name, start_date, end_date, currency_code)
     VALUES ($1, $2, $3, 'X', '2026-12-10', '2026-12-12', 'INR') RETURNING id`,
    [tenant, ty, code],
  );
  return r.rows[0]!.id;
}
...
describe('event_subteams — schema correctness (Phase 3 Unit 2)', () => {
  ...
  it('inserts a valid subteam', async () => {
    ...
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'Catering Team')`,
      [t, e],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_subteams`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects bad color_hex format', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO event_subteams (tenant_id, event_id, name, color_hex) VALUES ($1, $2, 'X', 'red')`,
      [t, e],
    );
  ...
  it('partial UNIQUE: blocks duplicate name per event (case-insensitive)', async () => {
    ...
    const e = await mkEvent(db, t);
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'Catering')`,
      [t, e],
    );
    const err = await tryExec(
      db,
  ...
  it('removed subteam frees the name slot', async () => {
    ...
    const e = await mkEvent(db, t);
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name, removed_at) VALUES ($1, $2, 'Catering', now())`,
      [t, e],
    );
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'Catering')`,
  ...
  it('partial UNIQUE: same lead cannot head two subteams on same event', async () => {
    ...
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name, lead_id) VALUES ($1, $2, 'A', $3)`,
      [t, e, m],
    );
    const err = await tryExec(
      db,
      `INSERT INTO event_subteams (tenant_id, event_id, name, lead_id) VALUES ($1, $2, 'B', $3)`,
      [t, e, m],
    );
    expect(err).toMatch(/duplicate|unique/i);
  });
  ...
  it('NULL leads do not conflict (multiple unled subteams allowed)', async () => {
    ...
    const e = await mkEvent(db, t);
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'A'), ($1, $2, 'B')`,
      [t, e],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_subteams`))
      .rows[0]!.c;
  ...
  it('trigger: rejects tenant_id mismatching events.tenant_id', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'X')`,
      [t2, e],
    );
  ...
  it('CASCADE: deleting event removes its subteams', async () => {
    const t = await mkTenant(db, 'acme-co');
    const e = await mkEvent(db, t);
    await db.query(`INSERT INTO event_subteams (tenant_id, event_id, name) VALUES ($1, $2, 'X')`, [
      t,
      e,
    ]);
  ...
  it('SET NULL: deleting lead member nulls lead_id but keeps the subteam', async () => {
    ...
    const m = await mkMember(db, t, 'lead@y.dev');
    await db.query(
      `INSERT INTO event_subteams (tenant_id, event_id, name, lead_id) VALUES ($1, $2, 'X', $3)`,
      [t, e, m],
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM event_subteams`)).rows.length,
    );
    expect(anon).toBe(0);
