...
describe('tenant_cohort_metrics — schema correctness (Phase 2 Unit 40)', () => {
  ...
  it('inserts a valid cohort row', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_cohort_metrics`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('same cohort, multiple measurement months coexist', async () => {
    ...
    const c = (await db.query<{ c: number }>(
      `SELECT count(*)::int AS c FROM tenant_cohort_metrics`)).rows[0]!.c;
    expect(c).toBe(3);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ cohort_month: string }>(`SELECT cohort_month FROM tenant_cohort_metrics`)).rows.length);
    expect(anon).toBe(0);
