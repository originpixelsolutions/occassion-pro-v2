...
describe('onboarding_state — schema correctness (Phase 8 Unit 55)', () => {
  ...
  it('inserts a valid state', async () => {
    ...
    await db.query(
      `INSERT INTO onboarding_state (tenant_id, signup_completed_at, current_tour_step, current_tour_step_index, completion_percent)
       VALUES ($1, now(), 'welcome', 1, 14)`, [t]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM onboarding_state`)).rows[0]!.c).toBe(1);
  });
  ...
  it('rejects bad current_tour_step', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, current_tour_step) VALUES ($1, 'expert_mode')`, [t]);
  ...
  it('completed AND skipped both set rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, tour_completed_at, tour_skipped_at) VALUES ($1, now(), now())`, [t]);
  ...
  it('template without timestamp rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, template_used) VALUES ($1, 'indian_wedding')`, [t]);
  ...
  it('demo_data_loaded=TRUE without timestamp rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, demo_data_loaded) VALUES ($1, TRUE)`, [t]);
  ...
  it('completion_percent > 100 rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, completion_percent) VALUES ($1, 110)`, [t]);
  ...
  it('workspace_setup before signup rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, signup_completed_at, workspace_setup_at)
       VALUES ($1, now(), now() - interval '1 hour')`, [t]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('rejects array checklist_progress', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO onboarding_state (tenant_id, checklist_progress) VALUES ($1, '[]'::jsonb)`, [t]);
  ...
  it('completed tour happy path', async () => {
    ...
    await db.query(
      `INSERT INTO onboarding_state (tenant_id, signup_completed_at, workspace_setup_at, first_event_created_at, tour_completed_at, current_tour_step_index, completion_percent, template_used, template_used_at)
       VALUES ($1, now() - interval '7 days', now() - interval '6 days', now() - interval '5 days', now() - interval '4 days', 7, 100, 'indian_wedding', now() - interval '5 days')`, [t]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM onboarding_state WHERE tour_completed_at IS NOT NULL`)).rows[0]!.c).toBe(1);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ tenant_id: string }>(`SELECT tenant_id FROM onboarding_state`)).rows.length);
    expect(anon).toBe(0);
