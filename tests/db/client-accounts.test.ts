...
describe('client_accounts — schema correctness (Phase 3 Unit 13)', () => {
  ...
  it('inserts a valid account', async () => {
    await db.query(
      `INSERT INTO client_accounts (email, full_name, phone, password_hash)
       VALUES ('a@y.dev', 'Alice', '+919876543210', $1)`,
      [PW_HASH],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM client_accounts`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });

  it('UNIQUE email blocks duplicate', async () => {
    await db.query(`INSERT INTO client_accounts (email, password_hash) VALUES ('a@y.dev', $1)`, [
      PW_HASH,
    ]);
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, password_hash) VALUES ('A@Y.DEV', $1)`,
      [PW_HASH],
    );
  ...
  it('rejects malformed email', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, password_hash) VALUES ('not-an-email', $1)`,
      [PW_HASH],
    );
  ...
  it('rejects bad phone (no +)', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, phone, password_hash) VALUES ('a@y.dev', '9876543210', $1)`,
      [PW_HASH],
    );
  ...
  it('mfa_coupling: enabled=TRUE without secret rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, password_hash, mfa_enabled) VALUES ('a@y.dev', $1, TRUE)`,
      [PW_HASH],
    );
  ...
  it('mfa_coupling: enabled=TRUE with secret accepted', async () => {
    await db.query(
      `INSERT INTO client_accounts (email, password_hash, mfa_enabled, mfa_secret)
       VALUES ('a@y.dev', $1, TRUE, '\\x00aabb'::bytea)`,
      [PW_HASH],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM client_accounts`))
      .rows[0]!.c;
  ...
  it('suspend_coupling: suspended_at without reason rejected', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, password_hash, suspended_at) VALUES ('a@y.dev', $1, now())`,
      [PW_HASH],
    );
  ...
  it('rejects negative failed_login_count', async () => {
    const err = await tryExec(
      db,
      `INSERT INTO client_accounts (email, password_hash, failed_login_count) VALUES ('a@y.dev', $1, -1)`,
      [PW_HASH],
    );
  ...
  it('citext email: lower-case stored, case-insensitive match', async () => {
    ...
    const r = await db.query<{ email: string }>(
      `SELECT email FROM client_accounts WHERE email = 'a@y.dev'`,
    );
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM client_accounts`)).rows.length,
    );
    expect(anon).toBe(0);
