...
describe('event_readiness_state — schema correctness (Phase 3 Unit 5)', () => {
  ...
  it('inserts a pending row (default state)', async () => {
    ...
    await db.query(
      `INSERT INTO event_readiness_state (event_id, item_id) VALUES ($1, $2)`, [e, i]);
  ...
  it('rejects is_complete=TRUE without completed_at', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_readiness_state (event_id, item_id, is_complete) VALUES ($1, $2, TRUE)`, [e, i]);
  ...
  it('trigger: rejects item whose event_type does not match event', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_readiness_state (event_id, item_id) VALUES ($1, $2)`, [e, i_other_type]);
  ...
  it('CASCADE: deleting event removes its state rows', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_readiness_state`)).rows[0]!.c;
    expect(c).toBe(0);
  });
