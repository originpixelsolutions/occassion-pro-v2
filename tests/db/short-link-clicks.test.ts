...
describe('short_link_clicks — schema correctness (Phase 3 Unit 25)', () => {
  ...
  it('inserts a valid click', async () => {
    ...
    await db.query(
      `INSERT INTO short_link_clicks (link_id, tenant_id, device_type, country_code, outcome)
       VALUES ($1,$2,'mobile','IN','success')`, [l, t]);
  ...
  it('rejects bad device_type', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO short_link_clicks (link_id, tenant_id, device_type) VALUES ($1,$2,'smartwatch')`, [l, t]);
  ...
  it('rejects bad outcome', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO short_link_clicks (link_id, tenant_id, outcome) VALUES ($1,$2,'maybe')`, [l, t]);
  ...
  it('rejects bad country_code (lowercase)', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO short_link_clicks (link_id, tenant_id, country_code) VALUES ($1,$2,'in')`, [l, t]);
  ...
  it('cross-tenant: click.tenant_id != link.tenant_id rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO short_link_clicks (link_id, tenant_id) VALUES ($1,$2)`, [l1, t2]);
  ...
  it('DELETE permitted (archive sweep)', async () => {
    ...
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM short_link_clicks`)).rows[0]!.c).toBe(0);
  });
  ...
  it('rejects oversize user_agent', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO short_link_clicks (link_id, tenant_id, user_agent) VALUES ($1,$2,$3)`, [l, t, longUa]);
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM short_link_clicks`)).rows.length);
    expect(anon).toBe(0);
