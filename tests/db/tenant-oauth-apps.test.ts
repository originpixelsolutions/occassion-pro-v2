...
describe('tenant_oauth_apps — schema correctness (Phase 2 Unit 21)', () => {
  ...
  it('inserts a valid app', async () => {
    ...
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_oauth_apps`))
      .rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('rejects malformed client_id', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'bad_id', $2, 'X', ARRAY['https://x.com/cb'], ARRAY['x'])`,
      [t, HASH64],
    );
  ...
  it('rejects wrong-length client_secret_hash', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', 'short', 'X', ARRAY['https://x.com/cb'], ARRAY['x'])`,
      [t],
    );
  ...
  it('rejects empty redirect_uris', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY[]::text[], ARRAY['x'])`,
      [t, HASH64],
    );
  ...
  it('rejects empty scopes', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY['https://x.com/cb'], ARRAY[]::text[])`,
      [t, HASH64],
    );
  ...
  it('trigger: rejects http redirect (non-localhost)', async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY['http://insecure.example/cb'], ARRAY['x'])`,
      [t, HASH64],
    );
  ...
  it('trigger: accepts http://localhost (dev)', async () => {
    ...
    await db.query(
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY['http://localhost:3000/cb'], ARRAY['x'])`,
      [t, HASH64],
    );
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM tenant_oauth_apps`))
      .rows[0]!.c;
  ...
  it("rejects 'suspended' without suspended_at + suspended_reason", async () => {
    ...
    const err = await tryExec(
      ...
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes, status)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY['https://x.com/cb'], ARRAY['x'], 'suspended')`,
      [t, HASH64],
    );
  ...
  it("rejects 'revoked' without revoked_at", async () => {
    ...
    const err = await tryExec(
      db,
      `INSERT INTO tenant_oauth_apps (tenant_id, client_id, client_secret_hash, name, redirect_uris, scopes, status)
       VALUES ($1, 'op_app_aaaabbbbccccdddd', $2, 'X', ARRAY['https://x.com/cb'], ARRAY['x'], 'revoked')`,
      [t, HASH64],
    );
    expect(err).toMatch(/check/i);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(
      db,
      'anon',
      async () => (await db.query<{ id: string }>(`SELECT id FROM tenant_oauth_apps`)).rows.length,
    );
    expect(anon).toBe(0);
