...
async function mkStorage(db: TestDb, tenant: string): Promise<string> {
  const r = await db.query<{ id: string }>(
    `INSERT INTO tenant_external_storage (tenant_id, provider, access_token_encrypted)
     VALUES ($1, 'r2', '\\x00aa'::bytea) RETURNING id`, [tenant]);
  return r.rows[0]!.id;
}
...
describe('event_offload_jobs — schema correctness (Phase 3 Unit 7)', () => {
  ...
  it('inserts a queued job (default state)', async () => {
    ...
    await db.query(
      `INSERT INTO event_offload_jobs (tenant_id, event_id, storage_id) VALUES ($1, $2, $3)`, [t, e, s]);
  ...
  it("rejects 'running' without started_at", async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id, status) VALUES ($1, $2, 'running')`, [t, e]);
  ...
  it("rejects 'completed' missing any of the four prereqs", async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id, status, started_at, completed_at)
       VALUES ($1, $2, 'completed', now(), now())`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it("rejects 'cancelled' without cancelled_at", async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id, status) VALUES ($1, $2, 'cancelled')`, [t, e]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('rejects negative bytes_offloaded / files_count', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id, bytes_offloaded) VALUES ($1, $2, -1)`, [t, e]);
  ...
  it('rejects attempt_count outside 0..50', async () => {
    ...
    const e = await mkEvent(db, t);
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id, attempt_count) VALUES ($1, $2, 51)`, [t, e]);
  ...
  it('partial UNIQUE: blocks second active job per event', async () => {
    ...
    const e = await mkEvent(db, t);
    await db.query(
      `INSERT INTO event_offload_jobs (tenant_id, event_id) VALUES ($1, $2)`, [t, e]);
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id) VALUES ($1, $2)`, [t, e]);
  ...
  it('cancelled job does NOT block a new queued job for same event', async () => {
    ...
    await db.query(
      `INSERT INTO event_offload_jobs (tenant_id, event_id, status, cancelled_at)
       VALUES ($1, $2, 'cancelled', now())`, [t, e]);
    await db.query(
      `INSERT INTO event_offload_jobs (tenant_id, event_id) VALUES ($1, $2)`, [t, e]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_offload_jobs`)).rows[0]!.c;
    expect(c).toBe(2);
  });
  ...
  it('trigger: rejects job whose tenant_id differs from event.tenant_id', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO event_offload_jobs (tenant_id, event_id) VALUES ($1, $2)`, [t2, e_t1]);
  ...
  it('happy path to completed', async () => {
    ...
    await db.query(
      `INSERT INTO event_offload_jobs (tenant_id, event_id, storage_id, status, started_at, completed_at, bytes_offloaded, files_count)
       VALUES ($1, $2, $3, 'completed', now() - interval '1 hour', now(), 1234567, 42)`, [t, e, s]);
    const c = (await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM event_offload_jobs`)).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('RLS pair', async () => {
    ...
    const anon = await withRole(db, 'anon', async () =>
      (await db.query<{ id: string }>(`SELECT id FROM event_offload_jobs`)).rows.length);
    expect(anon).toBe(0);
