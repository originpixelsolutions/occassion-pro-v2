  ...
  it('inserts a valid recovery code', async () => {
    await db.query(
      `INSERT INTO account_recovery_codes (user_id, user_type, code_hash)
       VALUES ($1, 'tenant_member', $2)`,
      [USER, hash('1')],
    );
    const c = (
      await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM account_recovery_codes`)
    ).rows[0]!.c;
    expect(c).toBe(1);
  });
  ...
  it('trigger: cap at 20 unconsumed codes per (user, user_type)', async () => {
    for (let i = 1; i <= 20; i++) {
      await db.query(
        `INSERT INTO account_recovery_codes (user_id, user_type, code_hash)
         VALUES ($1, 'tenant_member', $2)`,
        [USER, hash(String(i).padStart(3, '0'))],
      );
    }
    const err = await tryExec(
      db,
  ...
  it('trigger: consuming a code frees a slot', async () => {
    ...
    const active = (
      await db.query<{ c: number }>(
        `SELECT count(*)::int AS c FROM account_recovery_codes WHERE consumed_at IS NULL AND user_id = $1`,
        [USER],
      )
    ).rows[0]!.c;
