...
describe('RLS on rsvp_change_requests (Phase 12 Unit 96)', () => {
  ...
  it('tenant_member sees own tenant requests only', async () => {
    ...
    await db.query(
      `INSERT INTO rsvp_change_requests (tenant_id, event_id, guest_id, old_rsvp_status, new_rsvp_status, status) VALUES ($1, $2, $3, 'pending', 'attending', 'pending')`,
      [t1, e1, g1],
    );
    await db.query(
      `INSERT INTO rsvp_change_requests (tenant_id, event_id, guest_id, old_rsvp_status, new_rsvp_status, status) VALUES ($1, $2, $3, 'pending', 'attending', 'pending')`,
