...
async function mkNotif(db: TestDb, tenant: string): Promise<string> {
  return (await db.query<{ id: string }>(
    `INSERT INTO notifications (tenant_id, recipient_type, recipient_id, category, title, body)
     VALUES ($1,'client', gen_random_uuid(),'x','t','b') RETURNING id`, [tenant])).rows[0]!.id;
}
...
describe('notification_deliveries — schema correctness (Phase 8 Unit 53)', () => {
  ...
  it('inserts a valid queued delivery', async () => {
    ...
    await db.query(
      `INSERT INTO notification_deliveries (notification_id, channel, provider, recipient_address)
       VALUES ($1,'email','sendgrid','client@y.dev')`, [n]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM notification_deliveries`)).rows[0]!.c).toBe(1);
  });
  ...
  it('rejects bad channel', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel) VALUES ($1,'carrier_pigeon')`, [n]);
  ...
  it('attempts > 20 rejected', async () => {
    ...
    const n = await mkNotif(db, t);
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, attempts) VALUES ($1,'email', 21)`, [n]);
  ...
  it('sent requires sent_at', async () => {
    ...
    const n = await mkNotif(db, t);
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, status) VALUES ($1,'email','sent')`, [n]);
  ...
  it('delivered requires sent_at AND delivered_at', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, status, delivered_at)
       VALUES ($1,'email','delivered', now())`, [n]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('time ordering: sent before queued rejected', async () => {
    ...
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, queued_at, sent_at)
       VALUES ($1,'email', now(), now() - interval '1 hour')`, [n]);
    expect(err).toMatch(/check/i);
  });
  ...
  it('cost without currency rejected', async () => {
    ...
    const n = await mkNotif(db, t);
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, cost_micro_units) VALUES ($1,'sms', 12500)`, [n]);
  ...
  it('UNIQUE (channel, provider_message_id) blocks dupe', async () => {
    ...
    const n = await mkNotif(db, t);
    await db.query(`INSERT INTO notification_deliveries (notification_id, channel, provider_message_id) VALUES ($1,'whatsapp','wamid_X1')`, [n]);
    const err = await tryExec(db,
      `INSERT INTO notification_deliveries (notification_id, channel, provider_message_id) VALUES ($1,'whatsapp','wamid_X1')`, [n]);
  ...
  it('delivered happy path with cost', async () => {
    ...
    await db.query(
      `INSERT INTO notification_deliveries (notification_id, channel, status, queued_at, sent_at, delivered_at, cost_micro_units, cost_currency, provider, provider_message_id)
       VALUES ($1,'sms','delivered', now() - interval '10 minutes', now() - interval '5 minutes', now() - interval '4 minutes', 25000, 'INR','twilio','SM123')`, [n]);
    expect((await db.query<{ c: number }>(`SELECT count(*)::int AS c FROM notification_deliveries WHERE status='delivered'`)).rows[0]!.c).toBe(1);
  });

  it('RLS pair', async () => {
    const t = await mkTenant(db, 'nd-www');
    const n = await mkNotif(db, t);
    await db.query(`INSERT INTO notification_deliveries (notification_id, channel) VALUES ($1,'email')`, [n]);
