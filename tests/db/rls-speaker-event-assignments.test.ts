...
describe('RLS on speaker_event_assignments (Phase 12 Unit 89)', () => {
  ...
  it('tenant_member sees own tenant assignments only', async () => {
    ...
    await db.query(
      `INSERT INTO speaker_event_assignments (tenant_id, event_id, session_id, speaker_account_id, role, status, invited_by, invited_at) VALUES ($1, $2, $3, $4, 'speaker', 'invited', $5, now())`,
      [t1, e1, s1, sp1, u1],
    );
    await db.query(
      `INSERT INTO speaker_event_assignments (tenant_id, event_id, session_id, speaker_account_id, role, status, invited_by, invited_at) VALUES ($1, $2, $3, $4, 'speaker', 'invited', $5, now())`,
    ...
    const n = (await db.query<{ id: string }>(`SELECT id FROM speaker_event_assignments`)).rows
      .length;
  ...
  it('speaker sees own assignments only', async () => {
    ...
    await asRole(db, 'authenticated');
    const rows = await db.query<{ speaker_account_id: string }>(
      `SELECT speaker_account_id FROM speaker_event_assignments`,
    );
    await asSuperuser(db);
