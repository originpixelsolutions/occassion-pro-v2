...
--
-- Append-only activity stream per event. Any tenant_member
-- SELECT/INSERT (with attribution lock: actor_id must equal
-- current_user_id() when set). No UPDATE - activity rows
-- are immutable. DELETE owner/event_manager only (retention
-- pruning).
...
CREATE POLICY eaf_insert_member ON event_activity_feed
  FOR INSERT WITH CHECK (
    ...
    AND (actor_id IS NULL OR actor_id = current_user_id())
  );
