...
export const vendorReviews = pgTable(
  'vendor_reviews',
  {
    ...
    reviewerTenantId: uuid('reviewer_tenant_id').references(() => tenants.id, {
      onDelete: 'set null',
    }),
    eventId: uuid('event_id').references(() => events.id, { onDelete: 'set null' }),
    vendorAssignmentId: uuid('vendor_assignment_id').references(() => vendorEventAssignments.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => ({
    assignmentUq: uniqueIndex('uq_vendor_reviews_assignment')
      .on(t.vendorAssignmentId)
      .where(sql`${t.vendorAssignmentId} IS NOT NULL AND ${t.deletedAt} IS NULL`),
    vendorIx: index('idx_vendor_reviews')
      .on(t.vendorAccountId, t.createdAt)
      .where(sql`${t.deletedAt} IS NULL`),
    ratingRange: check('vr_rating_range', sql`${t.rating} >= 1.0 AND ${t.rating} <= 5.0`),
    publishCoupling: check(
      'vr_publish_coupling',
      sql`NOT ${t.isPublished} OR ${t.unpublishedAt} IS NULL`,
    ),
    unpublishReasonCoupling: check(
      'vr_unpublish_reason',
      sql`${t.unpublishedAt} IS NULL OR ${t.unpublishedReason} IS NOT NULL`,
    ),
    responseCoupling: check(
      'vr_response_coupling',
      sql`(${t.vendorResponse} IS NULL) = (${t.vendorRespondedAt} IS NULL)`,
    ),
    flagCoupling: check(
      'vr_flag_coupling',
      sql`(${t.flaggedAt} IS NULL) = (${t.flaggedReason} IS NULL)`,
    ),
  }),
);
