...
export const VENDOR_CREW_ROLES = [
  'head_chef','sous_chef','server','bartender','photographer','videographer',
  'sound_engineer','lighting_tech','stage_manager','decorator','driver','security','assistant','floor_manager','other',
] as const;
...
export const vendorCrewMembers = pgTable(
  ...
  (t) => ({
    vendorIx: index('idx_vendor_crew').on(t.vendorAccountId).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('vcm_status_enum', sql`${t.status} IN ('active','inactive')`),
    rateCurrencyCoupling: check('vcm_rate_currency', sql`(${t.hourlyRate} IS NULL) = (${t.currencyCode} IS NULL)`),
    inactiveCoupling: check('vcm_inactive_coupling', sql`${t.status} <> 'inactive' OR ${t.deactivatedAt} IS NOT NULL`),
  }),
);
