...
import { check, index, integer, jsonb, numeric, pgTable, text, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';
import { tenants } from './tenants.js';
import { tenantMembers } from './tenant-members.js';

export const INVENTORY_AUDIT_TYPES = ['periodic','spot_check','post_event','annual','triggered'] as const;
...
export const inventoryAudits = pgTable(
  'inventory_audits',
  {
    ...
    auditedAt: timestamp('audited_at', { withTimezone: true }).notNull().default(sql`now()`),
  ...
  (t) => ({
    ...
    typeEnum: check('iau_type_enum', sql`${t.auditType} IN ('periodic','spot_check','post_event','annual','triggered')`),
    statusEnum: check('iau_status_enum', sql`${t.status} IN ('in_progress','completed','cancelled')`),
    valueCurrencyCoupling: check('iau_value_currency', sql`(${t.totalValueAudited} IS NULL) = (${t.totalValueCurrency} IS NULL)`),
  }),
);
