...
export const INVENTORY_ALLOC_STATUSES = ['allocated','dispatched','returned','damaged','lost','cancelled'] as const;
...
export const inventoryAllocations = pgTable(
  'inventory_allocations',
  {
    ...
    allocatedAt: timestamp('allocated_at', { withTimezone: true }).notNull().default(sql`now()`),
    ...
    dispatchedBy: uuid('dispatched_by').references(() => tenantMembers.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    ...
    statusEnum: check('ia_status_enum', sql`${t.status} IN ('allocated','dispatched','returned','damaged','lost','cancelled')`),
    qtyInvariant: check('ia_qty_invariant', sql`${t.quantityDamaged} + ${t.quantityLost} + ${t.quantityReturned} <= ${t.quantity}`),
    damageCostCoupling: check('ia_damage_cost_coupling', sql`(${t.damageCost} IS NULL) = (${t.damageCurrency} IS NULL)`),
  }),
);
