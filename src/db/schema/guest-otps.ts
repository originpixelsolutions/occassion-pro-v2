...
export const GUEST_OTP_INVALIDATED_REASONS = ['max_attempts','superseded','manual','expired_rotation'] as const;
...
export const guestOtps = pgTable(
  ...
  (t) => ({
    ...
    expiryWindow: check('go_expiry_window', sql`${t.expiresAt} > ${t.createdAt} AND ${t.expiresAt} <= ${t.createdAt} + interval '15 minutes'`),
    invCoupling: check('go_inv_coupling', sql`(${t.invalidatedAt} IS NULL) = (${t.invalidatedReason} IS NULL)`),
    terminalXor: check('go_terminal_xor', sql`${t.consumedAt} IS NULL OR ${t.invalidatedAt} IS NULL`),
  }),
);
