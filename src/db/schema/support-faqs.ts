...
export const supportFaqs = pgTable(
  'support_faqs',
  {
    ...
    lastReviewedBy: uuid('last_reviewed_by').references(() => superAdmins.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => ({
    activeIx: index('idx_support_faqs_active')
      .on(t.category, t.sortOrder)
      .where(sql`${t.isActive} = TRUE AND ${t.deletedAt} IS NULL`),
    categoryEnum: check(
      'sf_category_enum',
      sql`${t.category} IS NULL OR ${t.category} IN ('billing','events','guests','vendors','clients','speakers','runsheet','floor_plan','inventory','communications','payments','exports','onboarding','account','security','integrations','technical','other')`,
    ),
    audienceEnum: check(
      'sf_audience_enum',
      sql`${t.audience} IN ('all','tenant_member','client','vendor','guest','speaker','super_admin')`,
    ),
    visibilityEnum: check(
      'sf_visibility_enum',
      sql`${t.visibility} IN ('public','authenticated','tenant_only','super_admin_only')`,
    ),
    activeRetiredCoupling: check(
      'sf_active_retired_coupling',
      sql`NOT ${t.isActive} OR ${t.retiredAt} IS NULL`,
    ),
    reviewCoupling: check(
      'sf_review_coupling',
      sql`(${t.lastReviewedAt} IS NULL) = (${t.lastReviewedBy} IS NULL)`,
    ),
  }),
);
