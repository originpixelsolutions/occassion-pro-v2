...
import { bigint, check, customType, index, integer, jsonb, numeric, pgTable, text, timestamp, uuid, type AnyPgColumn } from 'drizzle-orm/pg-core';
...
export const CLEANUP_SUGGESTION_TYPES = [
  'lite_archive_old','delete_duplicates','delete_old_exports','delete_old_pdfs',
  'offload_old','archive_old_events','compress_videos','dedupe_uploads','retention_purge',
] as const;
...
export const storageCleanupSuggestions = pgTable(
  'storage_cleanup_suggestions',
  {
    ...
    supersededBy: uuid('superseded_by').references((): AnyPgColumn => storageCleanupSuggestions.id, { onDelete: 'set null' }),
    metadata: jsonb('metadata'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().default(sql`now()`),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().default(sql`now()`),
  },
  (t) => ({
    tenantOpenIx: index('idx_cleanup_tenant_open').on(t.tenantId, t.generatedAt).where(sql`${t.status} = 'open'`),
    typeEnum: check('scs_type_enum', sql`${t.suggestionType} IN ('lite_archive_old','delete_duplicates','delete_old_exports','delete_old_pdfs','offload_old','archive_old_events','compress_videos','dedupe_uploads','retention_purge')`),
    statusEnum: check('scs_status_enum', sql`${t.status} IN ('open','dismissed','applied','superseded','expired')`),
    priorityEnum: check('scs_priority_enum', sql`${t.priority} IN ('low','normal','high','critical')`),
