...
import { boolean, check, index, integer, jsonb, pgTable, text, timestamp, uuid, type AnyPgColumn } from 'drizzle-orm/pg-core';
...
export const runsheetVersions = pgTable(
  'runsheet_versions',
  {
    ...
    diff: jsonb('diff'),
    baseVersionId: uuid('base_version_id').references((): AnyPgColumn => runsheetVersions.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    eventTimeIx: index('idx_runsheet_versions_event_time').on(t.eventId, t.createdAt).where(sql`${t.deletedAt} IS NULL`),
    noSelfBase: check('rv_no_self_base', sql`${t.id} <> ${t.baseVersionId}`),
    fullOrDiff: check('rv_full_or_diff', sql`
