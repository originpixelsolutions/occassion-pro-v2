...
export const NOTIF_PREF_USER_TYPES = [
  'tenant_member',
  'client',
  'vendor',
  'guest',
...
export const notificationPreferences = pgTable(
  ...
  (t) => ({
    ...
    userTypeEnum: check(
      'np_user_type_enum',
      sql`${t.userType} IN ('tenant_member','client','vendor','guest','speaker','super_admin')`,
    ),
    quietHoursCoupling: check(
      'np_quiet_hours_coupling',
      sql`(${t.quietHoursStart} IS NULL) = (${t.quietHoursEnd} IS NULL)`,
    ),
    tenantOrSuper: check(
      'np_tenant_or_super',
      sql`${t.tenantId} IS NOT NULL OR ${t.userType} = 'super_admin'`,
    ),
  }),
);
