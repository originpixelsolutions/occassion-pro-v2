...
export const outgoingWebhookDeliveries = pgTable(
  'outgoing_webhook_deliveries',
  {
    ...
    subscriptionId: uuid('subscription_id')
      .notNull()
      .references(() => outgoingWebhookSubscriptions.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    idempotencyUq: uniqueIndex('uq_webhook_deliveries_idempotency')
      .on(t.subscriptionId, t.idempotencyKey)
      .where(sql`${t.idempotencyKey} IS NOT NULL`),
    pendingIx: index('idx_webhook_deliveries_pending')
      .on(t.nextAttemptAt)
      .where(sql`${t.deliveredAt} IS NULL AND ${t.failedPermanently} = FALSE`),
    attemptRange: check(
      'owd_attempt_range',
      sql`${t.attemptCount} >= 0 AND ${t.attemptCount} <= 12`,
    ),
    delivVsFail: check(
      'owd_deliv_vs_fail',
      sql`${t.deliveredAt} IS NULL OR ${t.failedPermanently} = FALSE`,
    ),
    failCoupling: check(
      'owd_fail_coupling',
      sql`${t.failedPermanently} = FALSE OR (${t.failedPermanentlyAt} IS NOT NULL AND ${t.failedPermanentlyReason} IS NOT NULL)`,
    ),
  }),
);
