...
export const VENDOR_CREW_ASSIGN_STATUSES = [
  'scheduled',
  'confirmed',
  'checked_in',
  'checked_out',
...
export const vendorCrewAssignments = pgTable(
  'vendor_crew_assignments',
  {
    ...
    crewMemberId: uuid('crew_member_id')
      .notNull()
      .references(() => vendorCrewMembers.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    uqAssignmentMember: uniqueIndex('uq_vendor_crew_assignments').on(
      t.vendorAssignmentId,
      t.crewMemberId,
    ),
