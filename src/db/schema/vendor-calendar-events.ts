...
export const vendorCalendarEvents = pgTable(
  'vendor_calendar_events',
  {
    ...
    vendorCalendarId: uuid('vendor_calendar_id').notNull().references(() => vendorExternalCalendars.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    externalUq: uniqueIndex('uq_vendor_calendar_events_external').on(t.vendorCalendarId, t.externalEventId),
    timeIx: index('idx_vendor_cal_events_time').on(t.vendorCalendarId, t.startsAt, t.endsAt),
    busyIx: index('idx_vendor_cal_events_busy').on(t.vendorAccountId, t.startsAt, t.endsAt).where(sql`${t.busy} = TRUE AND ${t.status} <> 'cancelled'`),
