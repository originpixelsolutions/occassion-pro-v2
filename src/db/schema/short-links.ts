...
export const SHORT_LINK_TYPES = ['invitation','rsvp','portal','website','badge','asset','generic'] as const;
...
export const shortLinks = pgTable(
  ...
  (t) => ({
    ...
    activeIx: index('idx_short_links_active').on(t.code).where(sql`${t.isActive} = TRUE AND ${t.deletedAt} IS NULL`),
    typeEnum: check('sl_type_enum', sql`${t.linkType} IN ('invitation','rsvp','portal','website','badge','asset','generic')`),
