...
export const INVENTORY_CATEGORIES = [
  'tables','chairs','decor','av','lighting','kitchen','linen','crockery','signage','other',
] as const;
...
export const inventoryItems = pgTable(
  ...
  (t) => ({
    skuUq: uniqueIndex('uq_inventory_items_sku').on(t.tenantId, sql`lower(${t.sku})`)
      .where(sql`${t.sku} IS NOT NULL AND ${t.deletedAt} IS NULL`),
    tenantStatusIx: index('idx_inventory_tenant').on(t.tenantId, t.status).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('inv_status_enum', sql`${t.status} IN ('active','retired')`),
    qtyInvariant: check('inv_qty_invariant', sql`${t.quantityInStock} + ${t.quantityInUse} + ${t.quantityDamaged} <= ${t.quantityTotal}`),
    costCurrencyCoupling: check('inv_cost_currency', sql`(${t.unitCost} IS NULL AND ${t.unitReplacementCost} IS NULL) OR ${t.currencyCode} IS NOT NULL`),
    retiredCoupling: check('inv_retired_coupling', sql`${t.status} <> 'retired' OR ${t.retiredAt} IS NOT NULL`),
  }),
);
