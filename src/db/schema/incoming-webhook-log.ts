...
import { bigserial, boolean, check, index, integer, jsonb, pgTable, text, timestamp, uniqueIndex } from 'drizzle-orm/pg-core';

export const INCOMING_WEBHOOK_SOURCES = [
  'razorpay','stripe','cashfree','paytm','meta_whatsapp','sendgrid','twilio',
  'docusign','signwell','workos','google_calendar','outlook','zapier','generic_webhook',
] as const;
export type IncomingWebhookSource = (typeof INCOMING_WEBHOOK_SOURCES)[number];

export const INCOMING_WEBHOOK_STATUSES = ['received','processing','processed','failed','rejected','duplicate'] as const;
...
export const incomingWebhookLog = pgTable(
  ...
  (t) => ({
    sourceExternalUq: uniqueIndex('incoming_webhook_log_source_external_id_key').on(t.source, t.externalId),
    receivedIx: index('idx_incoming_webhook_received').on(t.receivedAt),
    sourceEnum: check('iwl_source_enum', sql`${t.source} IN ('razorpay','stripe','cashfree','paytm','meta_whatsapp','sendgrid','twilio','docusign','signwell','workos','google_calendar','outlook','zapier','generic_webhook')`),
    statusEnum: check('iwl_status_enum', sql`${t.status} IN ('received','processing','processed','failed','rejected','duplicate')`),
