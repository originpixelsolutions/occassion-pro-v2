...
import { boolean, check, customType, index, integer, pgTable, text, timestamp, uniqueIndex, uuid } from 'drizzle-orm/pg-core';
...
export const VENDOR_CALENDAR_PROVIDERS = ['google_calendar','outlook','apple_calendar','ical_url'] as const;
...
export const vendorExternalCalendars = pgTable(
  ...
  (t) => ({
    primaryUq: uniqueIndex('uq_vendor_external_calendars_primary')
      .on(t.vendorAccountId).where(sql`${t.isPrimary} = TRUE AND ${t.deletedAt} IS NULL`),
    vendorIx: index('idx_vendor_calendars_vendor').on(t.vendorAccountId).where(sql`${t.status} = 'active' AND ${t.deletedAt} IS NULL`),
    providerEnum: check('vec_provider_enum', sql`${t.provider} IN ('google_calendar','outlook','apple_calendar','ical_url')`),
    statusEnum: check('vec_status_enum', sql`${t.status} IN ('active','expired','disconnected','error')`),
    icalRequiresUrl: check('vec_ical_requires_url', sql`${t.provider} <> 'ical_url' OR ${t.icalUrl} IS NOT NULL`),
    tokensRequireKey: check('vec_tokens_require_key', sql`(${t.accessTokenEncrypted} IS NULL AND ${t.refreshTokenEncrypted} IS NULL) OR ${t.tokenKmsKeyId} IS NOT NULL`),
  }),
);
