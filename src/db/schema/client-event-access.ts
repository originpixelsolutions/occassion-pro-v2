...
export const CLIENT_EVENT_ACCESS_ROLES = [
  'primary',
  'secondary',
  'viewer',
  'approver',
...
export const clientEventAccess = pgTable(
  ...
  (t) => ({
    ...
    clientIx: index('idx_client_event_access_client')
      .on(t.clientAccountId)
      .where(sql`${t.revokedAt} IS NULL`),
    roleEnum: check(
      'cea_role_enum',
      sql`${t.role} IN ('primary','secondary','viewer','approver','signer')`,
    ),
    revokeCoupling: check(
      'cea_revoke_coupling',
      sql`${t.revokedAt} IS NULL OR (${t.revokedBy} IS NOT NULL AND ${t.revokedReason} IS NOT NULL)`,
    ),
  }),
);
