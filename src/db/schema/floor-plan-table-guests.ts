...
export const floorPlanTableGuests = pgTable(
  'floor_plan_table_guests',
  {
    tableId: uuid('table_id')
      .notNull()
      .references(() => floorPlanTables.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    ...
    seatUq: uniqueIndex('uq_floor_plan_table_guests_seat')
      .on(t.tableId, t.seatNumber)
      .where(sql`${t.seatNumber} IS NOT NULL`),
    guestIx: index('idx_fp_table_guests_guest').on(t.guestId),
    seatRange: check(
      'fptg_seat_range',
      sql`${t.seatNumber} IS NULL OR (${t.seatNumber} > 0 AND ${t.seatNumber} <= 100)`,
    ),
  }),
);
