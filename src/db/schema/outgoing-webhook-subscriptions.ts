...
export const outgoingWebhookSubscriptions = pgTable(
  ...
  (t) => ({
    activeIx: index('idx_webhook_subs_active')
      .on(t.tenantId)
      .where(sql`${t.isActive} = TRUE AND ${t.isPaused} = FALSE AND ${t.deletedAt} IS NULL`),
    urlFmt: check(
      'ows_url_fmt',
      sql`${t.url} ~ '^https://' AND length(${t.url}) BETWEEN 1 AND 2048`,
    ),
    algorithmEnum: check(
      'ows_algorithm_enum',
      sql`${t.signingAlgorithm} IN ('hmac_sha256','hmac_sha512')`,
    ),
    failuresLeDeliveries: check(
      'ows_failures_le_deliveries',
      sql`${t.totalFailures} <= ${t.totalDeliveries}`,
    ),
    pauseCoupling: check(
      'ows_pause_coupling',
      sql`${t.isPaused} = FALSE OR (${t.pausedAt} IS NOT NULL AND ${t.pausedReason} IS NOT NULL)`,
    ),
  }),
);
