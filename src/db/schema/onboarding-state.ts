...
export const onboardingState = pgTable(
  ...
  (t) => ({
    activeIx: index('idx_onboarding_state_active')
      .on(t.lastActiveAt)
      .where(sql`${t.tourCompletedAt} IS NULL AND ${t.tourSkippedAt} IS NULL`),
    tourStepEnum: check(
      'os_tour_step_enum',
      sql`${t.currentTourStep} IS NULL OR ${t.currentTourStep} IN ('welcome','branding','first_event','portal_tour','invite_team','payment','next_steps')`,
    ),
    tourTerminalXor: check(
      'os_tour_terminal_xor',
      sql`${t.tourCompletedAt} IS NULL OR ${t.tourSkippedAt} IS NULL`,
    ),
    templateCoupling: check(
      'os_template_coupling',
      sql`(${t.templateUsed} IS NULL) = (${t.templateUsedAt} IS NULL)`,
    ),
    completionRange: check(
      'os_completion_range',
      sql`${t.completionPercent} >= 0 AND ${t.completionPercent} <= 100`,
    ),
  }),
);
