...
export const CARD_BRANDS = [
  'visa','mastercard','amex','rupay','discover','diners','jcb','unionpay','other',
] as const;
...
export const tenantPaymentMethods = pgTable(
  ...
  (t) => ({
    ...
    expPair: check(
      'tpm_exp_pair',
      sql`(${t.expMonth} IS NULL) = (${t.expYear} IS NULL)`,
    ),
    ...
    yearBounds: check('tpm_year',  sql`${t.expYear}  IS NULL OR ${t.expYear}  BETWEEN 2024 AND 2099`),
  }),
);
