...
export const TICKET_STATUSES = ['open','bot_handled','escalated','in_progress','waiting_on_user','resolved','closed','reopened'] as const;
...
export const supportTickets = pgTable(
  'support_tickets',
  {
    ...
    botHandledFaqId: uuid('bot_handled_faq_id').references(() => supportFaqs.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    ...
    userTypeEnum: check('st_user_type_enum', sql`${t.userType} IN ('tenant_member','client','vendor','guest','speaker')`),
    priorityEnum: check('st_priority_enum', sql`${t.priority} IN ('low','normal','high','urgent')`),
    statusEnum: check('st_status_enum', sql`${t.status} IN ('open','bot_handled','escalated','in_progress','waiting_on_user','resolved','closed','reopened')`),
    ratingRange: check('st_rating_range', sql`${t.satisfactionRating} IS NULL OR (${t.satisfactionRating} BETWEEN 1 AND 5)`),
  }),
);
