...
import { bigserial, check, index, pgTable, text, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';
...
export const CLICK_OUTCOMES = ['success','password_required','password_failed','expired','inactive','not_found','rate_limited'] as const;
...
export const shortLinkClicks = pgTable(
  ...
  (t) => ({
    ...
    deviceEnum: check('slc_device_enum', sql`${t.deviceType} IS NULL OR ${t.deviceType} IN ('mobile','tablet','desktop','bot','other')`),
    outcomeEnum: check('slc_outcome_enum', sql`${t.outcome} IN ('success','password_required','password_failed','expired','inactive','not_found','rate_limited')`),
  }),
);
