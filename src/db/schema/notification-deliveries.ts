...
export const notificationDeliveries = pgTable(
  ...
  (t) => ({
    ...
    statusQueuedIx: index('idx_notification_deliveries_status')
      .on(t.status)
      .where(sql`${t.status} = 'queued'`),
    channelEnum: check(
      'nd_channel_enum',
      sql`${t.channel} IN ('in_app','email','push','sms','whatsapp','slack','teams')`,
    ),
    statusEnum: check(
      'nd_status_enum',
      sql`${t.status} IN ('queued','sending','sent','delivered','read','failed','bounced','suppressed')`,
    ),
    attemptsRange: check('nd_attempts_range', sql`${t.attempts} >= 0 AND ${t.attempts} <= 20`),
    costCoupling: check(
      'nd_cost_coupling',
      sql`(${t.costMicroUnits} IS NULL) = (${t.costCurrency} IS NULL)`,
    ),
  }),
);
