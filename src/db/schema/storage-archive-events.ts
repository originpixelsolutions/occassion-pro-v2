...
import { bigint, check, index, integer, jsonb, numeric, pgTable, text, timestamp, uuid } from 'drizzle-orm/pg-core';
import { tenants } from './tenants.js';
import { events } from './events.js';
import { tenantMembers } from './tenant-members.js';

export const ARCHIVE_TYPES = ['lifecycle','manual','tenant_request','quota_pressure','retention_policy','offload'] as const;
export type ArchiveType = (typeof ARCHIVE_TYPES)[number];

export const ARCHIVE_STATUSES = ['in_progress','completed','restoring','restored','purged','failed'] as const;
...
export const storageArchiveEvents = pgTable(
  'storage_archive_events',
  {
    ...
    restoreRequestedBy: uuid('restore_requested_by').references(() => tenantMembers.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    ...
    typeEnum: check('sae_type_enum', sql`${t.archiveType} IN ('lifecycle','manual','tenant_request','quota_pressure','retention_policy','offload')`),
    statusEnum: check('sae_status_enum', sql`${t.status} IN ('in_progress','completed','restoring','restored','purged','failed')`),
    bytesPositive: check('sae_bytes_positive', sql`${t.bytesArchived} > 0`),
    filePositive: check('sae_file_positive', sql`${t.fileCount} > 0`),
    windowAfterArchive: check('sae_window_after_archive', sql`${t.restoreWindowEndsAt} > ${t.archivedAt}`),
  }),
);
