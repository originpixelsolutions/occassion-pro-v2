...
import { check, index, integer, jsonb, numeric, pgTable, text, timestamp, uuid, varchar, type AnyPgColumn } from 'drizzle-orm/pg-core';
...
export const VENDOR_QUOTE_STATUSES = [
  'draft','submitted','tenant_approved','tenant_rejected','client_approved','client_rejected','expired','superseded',
] as const;
...
export const vendorQuotes = pgTable(
  'vendor_quotes',
  {
    ...
    tenantReviewedBy: uuid('tenant_reviewed_by').references(() => tenantMembers.id, { onDelete: 'set null' }),
    ...
    supersededBy: uuid('superseded_by').references((): AnyPgColumn => vendorQuotes.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    statusIx: index('idx_vendor_quotes_status_time').on(t.status, t.createdAt).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('vq_status_enum', sql`${t.status} IN ('draft','submitted','tenant_approved','tenant_rejected','client_approved','client_rejected','expired','superseded')`),
