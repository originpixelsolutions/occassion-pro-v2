...
export const invitations = pgTable(
  ...
  (t) => ({
    activeUq: uniqueIndex('uq_invitations_event_template_active')
      .on(t.eventId, t.templateCode)
      .where(sql`${t.deletedAt} IS NULL`),
    eventIx: index('idx_invitations_event')
      .on(t.eventId)
      .where(sql`${t.deletedAt} IS NULL`),
    variantEnum: check('inv_variant_enum', sql`${t.variant} IN ('static','animated_web','video')`),
    publishCoupling: check(
      'inv_publish_coupling',
      sql`${t.isPublished} = FALSE OR (${t.publishedAt} IS NOT NULL AND ${t.publishedBy} IS NOT NULL)`,
    ),
    videoRequiresUrl: check(
      'inv_video_requires_url',
      sql`${t.variant} <> 'video' OR ${t.videoUrl} IS NOT NULL`,
    ),
  }),
);
