...
import { boolean, check, customType, integer, jsonb, numeric, pgTable, text, timestamp, uniqueIndex, uuid, varchar } from 'drizzle-orm/pg-core';
...
export const vendorPortfolios = pgTable(
  'vendor_portfolios',
  {
    vendorAccountId: uuid('vendor_account_id').primaryKey().references(() => vendorAccounts.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    ...
    visibilityEnum: check('vp_visibility_enum', sql`${t.visibility} IN ('private','tenants_only','public')`),
    priceCurrencyCoupling: check('vp_price_currency', sql`(${t.startingPrice} IS NULL) = (${t.startingCurrency} IS NULL)`),
    ratingCountCoupling: check('vp_rating_count', sql`(${t.avgPerformanceRating} IS NULL AND ${t.totalRatingsCount} = 0) OR (${t.avgPerformanceRating} IS NOT NULL AND ${t.totalRatingsCount} > 0)`),
    verifiedCoupling: check('vp_verified_coupling', sql`NOT ${t.isVerified} OR (${t.verifiedAt} IS NOT NULL AND ${t.verifiedBy} IS NOT NULL)`),
  }),
);
