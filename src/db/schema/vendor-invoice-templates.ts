...
export const vendorInvoiceTemplates = pgTable(
  ...
  (t) => ({
    defaultUq: uniqueIndex('uq_vendor_invoice_templates_default')
      .on(t.vendorAccountId).where(sql`${t.isDefault} = TRUE AND ${t.deletedAt} IS NULL`),
    nameUq: uniqueIndex('uq_vendor_invoice_templates_name')
      .on(t.vendorAccountId, sql`lower(${t.name})`).where(sql`${t.deletedAt} IS NULL`),
    vendorIx: index('idx_vendor_templates').on(t.vendorAccountId).where(sql`${t.deletedAt} IS NULL`),
    typeEnum: check('vit_type_enum', sql`${t.templateType} IN ('html','pdf_overlay','docx')`),
    htmlRequiresContent: check('vit_html_requires_content', sql`${t.templateType} <> 'html' OR ${t.templateHtml} IS NOT NULL`),
    fileRequiresKey: check('vit_file_requires_key', sql`${t.templateType} = 'html' OR ${t.templateFileR2Key} IS NOT NULL`),
    activeRetiredCoupling: check('vit_active_retired_coupling', sql`NOT ${t.isActive} OR ${t.retiredAt} IS NULL`),
  }),
);
