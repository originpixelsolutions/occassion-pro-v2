...
export const PAYOUT_STATUSES = ['scheduled','approved','disbursing','disbursed','failed','cancelled'] as const;
export type PayoutStatus = (typeof PAYOUT_STATUSES)[number];

export const PAYOUT_GATEWAYS = ['razorpay_x','stripe_connect','cashfree_payout','manual','bank_transfer','cheque'] as const;
export type PayoutGateway = (typeof PAYOUT_GATEWAYS)[number];

export const PAYOUT_MILESTONE_TYPES = ['booking_advance','progress','final','retainer','expense_reimbursement','bonus','other'] as const;
...
export const vendorPayouts = pgTable(
  'vendor_payouts',
  {
    ...
    vendorAccountId: uuid('vendor_account_id').references(() => vendorAccounts.id, { onDelete: 'set null' }),
    assignmentId: uuid('assignment_id').references(() => vendorEventAssignments.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    gatewayUq: uniqueIndex('uq_vendor_payouts_gateway')
      .on(t.gateway, t.gatewayPayoutId).where(sql`${t.gatewayPayoutId} IS NOT NULL`),
    statusScheduledIx: index('idx_payouts_status_scheduled')
      .on(t.status, t.scheduledFor).where(sql`${t.status} IN ('scheduled','approved') AND ${t.deletedAt} IS NULL`),
    statusEnum: check('vp_status_enum', sql`${t.status} IN ('scheduled','approved','disbursing','disbursed','failed','cancelled')`),
