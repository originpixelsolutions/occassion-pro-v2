...
export const floorPlanTables = pgTable(
  ...
  (t) => ({
    numberUq: uniqueIndex('uq_floor_plan_tables_number')
      .on(t.floorPlanId, sql`lower(${t.tableNumber})`)
      .where(sql`${t.deletedAt} IS NULL`),
    planIx: index('idx_floor_plan_tables_plan')
      .on(t.floorPlanId)
      .where(sql`${t.deletedAt} IS NULL`),
    shapeEnum: check(
      'fpt_shape_enum',
      sql`${t.tableShape} IN ('round','rectangular','cocktail','banquet_row','square','oval')`,
    ),
    seatCountRange: check(
      'fpt_seat_count_range',
      sql`${t.seatCount} > 0 AND ${t.seatCount} <= 100`,
    ),
    rotationRange: check(
      'fpt_rotation_range',
      sql`${t.rotationDeg} >= 0 AND ${t.rotationDeg} < 360`,
    ),
  }),
);
