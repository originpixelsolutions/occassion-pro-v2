...
export const RUNSHEET_TASK_STATUSES = ['pending','blocked','in_progress','completed','cancelled'] as const;
...
export const runsheetTasks = pgTable(
  'runsheet_tasks',
  {
    ...
    parentTaskId: uuid('parent_task_id').references((): AnyPgColumn => runsheetTasks.id, { onDelete: 'set null' }),
    dependsOnId: uuid('depends_on_id').references((): AnyPgColumn => runsheetTasks.id, { onDelete: 'set null' }),
  ...
  (t) => ({
    eventTimeIx: index('idx_runsheet_tasks_event_time').on(t.eventId, t.scheduledStart).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('rt_status_enum', sql`${t.status} IN ('pending','blocked','in_progress','completed','cancelled')`),
    priorityEnum: check('rt_priority_enum', sql`${t.priority} IN ('low','normal','high','critical')`),
