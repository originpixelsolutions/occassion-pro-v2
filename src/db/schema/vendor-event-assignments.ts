...
export const VENDOR_ASSIGNMENT_STATUSES = ['invited','accepted','declined','completed','cancelled'] as const;
...
export const vendorEventAssignments = pgTable(
  ...
  (t) => ({
    ...
    eventStatusIx: index('idx_vendor_assignments_status').on(t.eventId, t.status).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('vea_status_enum', sql`${t.status} IN ('invited','accepted','declined','completed','cancelled')`),
    valueCurrencyCoupling: check('vea_value_currency_coupling', sql`(${t.contractValue} IS NULL) = (${t.currencyCode} IS NULL)`),
    ratingRequiresCompleted: check('vea_rating_requires_completed', sql`${t.performanceRating} IS NULL OR ${t.status} = 'completed'`),
  }),
);
