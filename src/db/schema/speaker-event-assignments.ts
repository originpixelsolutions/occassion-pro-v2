...
export const SPEAKER_ASSIGNMENT_STATUSES = [
  'invited',
  'confirmed',
  'declined',
  'cancelled',
...
export const speakerEventAssignments = pgTable(
  'speaker_event_assignments',
  {
    ...
    speakerAccountId: uuid('speaker_account_id')
      .notNull()
      .references(() => speakerAccounts.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    activeUq: uniqueIndex('uq_speaker_event_assignments_active')
      .on(t.speakerAccountId, t.sessionId)
      .where(sql`${t.deletedAt} IS NULL`),
    eventStatusIx: index('idx_speaker_assignments_status')
      .on(t.eventId, t.status)
      .where(sql`${t.deletedAt} IS NULL`),
    roleEnum: check(
      'sea_role_enum',
      sql`${t.role} IN ('speaker','moderator','panelist','keynote')`,
    ),
    statusEnum: check(
      'sea_status_enum',
      sql`${t.status} IN ('invited','confirmed','declined','cancelled')`,
    ),
    honorariumCoupling: check(
      'sea_honorarium_coupling',
      sql`(${t.honorarium} IS NULL) = (${t.currencyCode} IS NULL)`,
    ),
  }),
);
