...
export const INVOICE_STATUSES = ['draft','sent','viewed','paid','partially_paid','overdue','cancelled','refunded'] as const;
...
export const invoices = pgTable(
  ...
  (t) => ({
    ...
    statusIx: index('idx_invoices_status').on(t.tenantId, t.status, t.createdAt).where(sql`${t.deletedAt} IS NULL`),
    statusEnum: check('inv_status_enum', sql`${t.status} IN ('draft','sent','viewed','paid','partially_paid','overdue','cancelled','refunded')`),
    grandTotalMath: check('inv_grand_total_math', sql`${t.grandTotal} = ${t.subtotal} + ${t.taxTotal} - ${t.discountTotal}`),
